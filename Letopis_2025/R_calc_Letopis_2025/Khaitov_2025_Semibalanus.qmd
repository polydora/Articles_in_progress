---
title: "Характеристика популяции *Semibalanus balanoides* на литорали острова Ряжкова летом 2025 г."
author: "В. М. Хайтов"
format:
  docx:
...
**Хайтов В. М. Характеристика популяции Semibalanus balanoides на литорали острова Ряжкова летом 2025 г.** // Толмачева Е. Л.  (ред.) Летопись природы Кадалакшского заповедника за 2025 год (ежегодный отчет). Кандалакша. Т.1 (Летопись природы Кандалакшского заповедника, кн. ++)
    
Рассматриваются данные по учету обилия сеголеток и взрослых балянусов на литоральных валунах летом 2024 г.
    
**Khaitov V.M.  Characterization of the population of Semibalanus balanoides on the littoral of Ryazhkov Island in the summer of 2025.** // Tolmacheva E. L. (ed.)  The Chronicle of Nature by the Kandalaksha Reserve for 2025 (Annual report).  Kandalaksha. V.1. (The Chronicle of Nature by the Kandalaksha Reserve, Book N ++)
    
Data on abundance counts of juveniles and adult barnacle on littoral boulders in the summer of 2024 are reviewed.
:::
...
bal <- read_excel("data/Balanus_Ryazhkov_2012-2025.xlsx")
...
Описание поселений морских желудей производилось силами волонтеров. На четырех стандартных участках литорали  острова Ряжкова (Южная губа, Восточный берег, Северная губа и Западный берег) проводятся ежегодные описания популяции усоногих раков *Semibalanus balanoides*. На каждом из участков ежегодно выбирается случайным образом по несколько валунов. Координаты точек расположения валунов засекаются с помощью спутникового навигатора. Расположение точек сбора материала, обследованных  в `r Year` году приведено на  @fig-Stones. Координаты обследованных в `r Year` г камней приведены в @tbl-Stones.


Валуны, использованные в анализе, располагались не выше пояса фукоидов и имели в диаметре не менее 1 метра. Обилие балянусов подсчитывалось на двух сторонах валуна (обращенной к морю (“Sea”) и обращенной к суше (“Land”). На каждой стороне было заложено по три площадки 10х10 см, располагавшихся на высоте 30 см от поверхности грунта. Рамка, ограничивающая площадку, была разбита на 25 квадратов со стороной 2 см. В трех случайно выбранных квадратах было подсчитано обилие балянусов следующих классов: Живые сеголетки, Мертвые сеголетки, Живые особи возрастом 1+ и более (далее взрослые), Мертвые взрослые особи. Данные, по учетам балянусов в `r Year` году, приведены в @tbl-Data. 
...
``` {r}
#| label: fig-Stones
#| fig-cap: "Расположение камней, на которых проводился учет балянусов, на побережье о. Ряжкова (2025 г.). Position of stones where barnacles were counted on the shore of Ryazhkov island in 2025"


ggmap(Ryazh_map,  base_layer=ggplot(data=bal_25, aes(x=E, y=N))) + 
...
@fig-Abund демонстрирует обилие разных стадий на разных точках в `r Year` г. В этом году распределение обилия молоди по разным участкам побережья острова было более равномерным, чем в предыдущие годы.  


<!-- Анализ многолетних тенденций изменения численности молоди балянусов показал, что обилие осевшей молоди демонстрирует  -->
...
```{r}
#| label: fig-Abund
#| fig-cap: "Обилие сеголеток и взрослых балянусов (экз. на 4 кв.см.) в 2025 году. Abundance of juveniles and adult barnacles (ind per 4 sq. cm) in 2025."

bal_melt2 <-  bal_melt %>% filter(Stage %in% c("Живые сеголетки", "Живые взрослые")) %>% filter(Year == 2025)
...
<!--   bal_melt %>%  -->
<!--   filter(Stage == "Живые сеголетки") %>% -->
<!--     group_by(Year, Site, N_stone) %>%  -->
<!--     summarise(Abund_mean = mean(Abundance)) %>%  -->
<!--     group_by(Year, Site) %>%  -->
<!--     summarise(Abund = mean(Abund_mean)) %>%  -->
<!--     group_by(Year) %>%  -->
<!--     summarise(Mean = mean(Abund), Sd = sd(Abund), CV = Sd/Mean) %>%  -->
...
```{r}
#| label: tbl-Stones
#| tbl-cap: "Координаты камней, на которых проводился учет балянусов в 2025 г. Coordinates of stones where barnacles were counted in 2025."
...
```{r}
#| label: tbl-Data
#| tbl-cap: "Обилие сеголеток и взрослых балянусов  на побережье о. Ряжкова в 2025 г. (экз. на 4 кв.см.).  Abundance of barnacles juveniles adults on the shore of Ryazhkov island in 2025 (ind. per 4 sq. cm)"
