...
kable(tidy(anova(fit.genotype)$table), caption = "Таблица 1. Результаты дисперсионного анализа зависимости формы раковины от пола и генотипа мииди (вероятность встречи генов Ga)")
```
...
```{r}
kable(tidy(anova(fit.genotype_Ed)$table), caption = "Таблица 2. Результаты дисперсионного анализа зависимости формы раковины от пола и генотипа мииди (вероятность встречи генов Ed)")
...
Pl_pca <- ggplot(PC_score_myt_gpa, aes(x = Comp2, y = Comp1)) + 
  geom_point(aes(fill = ids_morph$Sex, size = ids_morph$Ga), shape = 21) + 
  scale_fill_manual(values = c("pink", "blue")) + 
  xlim(-0.5, 0.5) +
  labs(fill = "Пол", size = "Ga Structure", x = paste("PC 2 (",PC2_prop, "%)", sep =""), y = paste("PC 1 (",PC1_prop, "%)", sep =""))  + 
