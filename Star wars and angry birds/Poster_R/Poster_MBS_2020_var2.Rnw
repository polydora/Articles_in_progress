...
theme_set(theme_bw() + theme(axis.title = element_text(size = 20), plot.title = element_text(hjust = 0.5, size = 20)))
...
Pl_N_dead_N_alive_hem <- ggplot(hem_abund_bank, aes(x = N_dead, y = N_alive)) + geom_point(size = 4) + labs(x = "Мертвые (экз./кв.м)", y = "Живые (экз./кв.м)") + geom_smooth(method = "glm", method.args = list(family = "quasipoisson"), sixe = 1.5) 
