...
```{r fig.cap = "Рисунок +. Многолетние изменения количества видов, имеющих высокую встречаемость. Кривая - линия тренда, подобранная методом LOESS."}
...
Не уверен, что можно вытянуть статистикой какие-то значимые результаты, но глазом видно, что макисмумы в этой кривой приходятся на годы с минимумами пустых кишечников


На мой взгляд, вместо количества видов, имеющих встречаемость выше порога, лучше использовать какой-нибудь индекс разнообразия. Нверняка в науке о питании есть такие индексы. Специально не искал. Однако идейно близок будет индекс разнообразия Шеннона, котрый в данном контексте будет иметь вот такой вид:
...
Для сравнения привожу все три графика на одном рисунке

```{r}
plot_grid(Pl_Year, Pl_Abund_species, Pl_H, ncol = 1)
```
