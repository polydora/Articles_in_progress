...
finch_il$Date <- as.Date(finch_il$Date)
...
# Вставляю исправленный файл

df_final <- read.table("Data/df_final_Parasitemia.csv", sep = ";", dec = ",", header = TRUE)

df_final$Experiment <- factor(df_final$Experiment, labels = c("Control", "SGS1", "GRW2"))

df_final$Ring <- factor(df_final$Ring)
...
df_final2 <- read.table("Data/df_final2_IL.csv", sep = ";", dec = ",", header = TRUE)

df_final2$Experiment <- factor(df_final2$Experiment, labels = c("Control", "SGS1", "GRW2"))

df_final2$Ring <- factor(df_final2$Ring)

df_final2$Date <- as.Date(df_final2$Date,format = "%d.%m.%Y" )
...
**Внимание!** При расчетах я использовал те файлы, которые прислала Мария! То есть в качестве датафреймов  df_final и df_final2 я использовал не результаты вычислений по коду, а то, что прислали. Как я понял там находятся какие-то результаты ручного персчета. 
