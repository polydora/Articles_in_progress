...
finch_il$Experiment <- factor(finch_il$Experiment, labels = c("Control", "pSGS1", "pGRW2"))
...
finch$Experiment <- factor(finch$Experiment, labels = c("Control", "pSGS1", "pGRW2"))
...
```{r}
library(broom)
kable(tidy(M2_Par_gam, parametric = T ))
```

smoothers

```{r}
kable(tidy(M2_Par_gam,parametric = F ))
```
...
```{r}
My_data <- expand.grid(Experiment = c("pSGS1", "pGRW2"), DPI = seq(0, max(finch_final$DPI)))
...
ggplot(My_data, aes(x = DPI, y = Fit)) + 
  ...
  geom_point(data = finch_il_2, aes(y = Parasitemia, color = Experiment)) +
...
```{r}
comp1 <- smooth_diff(M2_Par_gam, My_data, 'pSGS1', 'pGRW2', 'Experiment', unconditional = TRUE)
...
```{r}
plot_grid(Pl_parasetemia, Pl_Parasitemia_difference, labels = "AUTO")
```
...
```{r}
kable(tidy(M2_RMR_gam, parametric = T))
```
...
comp1 <- smooth_diff(M2_RMR_gam, My_data, 'Control', 'pSGS1', 'Experiment',unconditional = TRUE)

comp2 <- smooth_diff(M2_RMR_gam, My_data, 'Control', 'pGRW2', 'Experiment',unconditional = TRUE)

comp3 <- smooth_diff(M2_RMR_gam, My_data, 'pSGS1', 'pGRW2', 'Experiment',unconditional = TRUE)

comp <- cbind(DPI = seq(0, max(finch_final$DPI)),
              rbind(comp1, comp2, comp3))

comp$pair <- factor(comp$pair, levels = c("Control-pSGS1", "Control-pGRW2", "pSGS1-pGRW2"))
...
```{r}
plot_grid(Pl_RMR, Pl_RMR_difference, labels = "AUTO")
...
```{r}
cont_exp1 <- comp %>% filter(pair == "Control-pSGS1") %>% filter(upper > 0 & lower > 0) %>% pull(DPI) %>% range()

cont_exp2 <- comp %>% filter(pair == "Control-pGRW2") %>% filter(upper > 0 & lower > 0) %>% pull(DPI) %>% range()


exp1_exp2 <- comp %>% filter(pair == "pSGS1-pGRW2") %>% filter(upper < 0 & lower < 0) %>% pull(DPI) %>% range()
```
...
```{r}
kable(tidy(M2_IL_gam, parametric = T))
```
...
ggplot(My_data, aes(x = DPI, y = 10^Fit)) + 
  geom_line(aes(color = Experiment), size = 1) + 
  geom_ribbon(aes(ymin = 10^(Fit - 1.96*SE), ymax = 10^(Fit + 1.96*SE)), alpha = 0.2) +
  geom_point(data = finch_il, aes(y = (IL6_pg_ml), color = Experiment)) +
...
```{r}
comp1 <- smooth_diff(M2_IL_gam, My_data, 'Control', 'pSGS1', 'Experiment', unconditional = TRUE)

comp2 <- smooth_diff(M2_IL_gam, My_data, 'Control', 'pGRW2', 'Experiment', unconditional = TRUE)

comp3 <- smooth_diff(M2_IL_gam, My_data, 'pSGS1', 'pGRW2', 'Experiment', unconditional = TRUE)
...
```{r}
plot_grid(Pl_IL, Pl_IL_difference, labels = "AUTO")
```
