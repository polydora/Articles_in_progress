...
Pl_Obskaya_bay + geom_point(data = hotspot,  aes(x = Lon, y = Lat, group =1), size = 4, shape = 22, fill = "yellow") + geom_point(data = forecast_points, aes(x = lon, y = lat, group =1))
...
# options(sdmpredictors_datadir="D:/Data_LMBE/Obskaya Bay additional data/sdmpredictors")
...
library(raster)
my.sites.environment <- data.frame(species = my.sites$species, Depth=extract(bathymetry,my.sites[,2:3]) , extract(environment.bottom,my.sites[,2:3]), Status = my.sites$Status)
...
Используя базу [данных по инвазивным видам России](http://www.sevin.ru/top100worst/) и публикацию Goldsmit et al. (2019) я вытащил некоторый набор видов, которые обладают следующими признаками:

1. Они потенциально способны выдерживать опреснение (здесь надо еще внимательно фильтровать).
2. Они уже были отмечены, как инвазивные виды в других местообитаниях, как на территории России, так и в Канадской Арктике (возможно надо еще внимательнее посмотреть и кого-то выкинуть или добавить).

Далее такие виды будут обозначаться как Potentialy Non Indigenous Species (**PNIS**).

Используя данные бентосной съемки в Обской губе, я отфильтровал те виды, которые наиболее обильны на тех участков акватории, где расположены порты или для более опресненных участков, расположенных на юге акватории. Морскую часть сообщества, как мне видится, рассматривать не следует, так как вся морская фауна, будучи завезенной в сильно опресненные акватории портов, однозначно погибнет. 
Далее местные виды, будут обозначаться как нативные (**Native**).

Для всех этих видов, используя базу [GBIF](https://www.gbif.org/) я вытащил координаты точек, где эти виды были обнаружены. 

Уже простое сравнение частотных распределений широт для видов дает некоторую возможность отфильтровать потенциальных вселенцев. 
...
Pl_native <- ggplot(df_species_native, aes(y = decimalLatitude, x = species)) + geom_boxplot() + facet_wrap(~Status) + geom_hline(yintercept = 66.5622, linetype = 2) + theme(axis.text.x = element_text(angle = 90)) + ylim(-50, 90)

Pl_PNIS <- ggplot(df_species_PNIS, aes(y = decimalLatitude, x = species)) + geom_boxplot() + facet_wrap(~Status) + geom_hline(yintercept = 66.5622, linetype = 2) + theme(axis.text.x = element_text(angle = 90)) + ylim(-50, 90) 
...
Все эти характеристики были использованы в компонентном анализе, на основании которого была построена ординация видов.
...
ggplot(species_rda, aes(PC1, PC2, color = Status)) + geom_text(aes(label = species), size = 3) + scale_color_manual(values = c("blue", "red"))


```


Видно, что нативные виды обладают достаточно сходными биогеографическими характеристиками. Исключение составляет  Limnodrillus hoffmeisteri. Либо это ошибка в определнии, либо это космополит. 

По своим биогеографическим параметрам в число близких к нативным видам попадают следующие PNIS: `r species_rda %>% filter((PC1 > -1 & PC2 > -1) & Status == "PNIS" ) %>% pull(species)`. 

Из этих видов явно надо исключать Chionoecetes opilio и Botryllus schlosseri, как виды чисто морские. Остальные же вполне могут заселиться. 


Важно отметить, что это картинка получена лишь на основе биогеографических данных. Если дастся добавить сюда еще оценку соленостной и температурной толерантности видов, то она может существенно измениться. 
