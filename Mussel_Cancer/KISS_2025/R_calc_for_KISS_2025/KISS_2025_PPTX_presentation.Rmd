...
include_graphics("./figure/BTN_transmission_2.png")
...
- $MtrBTN2$ широко распространен и найден у нескольких видов мидий в Атлантике и в Пацифике в обоих полушариях
- $MtrBTN1$ отмечен только у *M.trossulus* в субарктике
...
- Для разных линий $BTN$ можно охарактеризоать их экологические ниши, т.е. выявить связи со средовыми предикторами, включая характеристики популяций хозяев, как это делается при построении моделей ниш «обычных» видов
...
- Линии $MtrBTN1$ и $MtrBTN2$ будучи по происхождению клетками  одного «родительского» вида (*M.trossulus*) и близкого эволюционного возраста должны иметь сходные связи с факторами среды.
...
- Cглаживающие функции подобраны для $MtrBTN1$ и $MtrBTN2$ отдельно


## Регрессионная модель: *предикторы значимо влияют на заболеваемость $MtrBTN1$ и не влияют на заболеваемость $MtrBTN2$*
...
```{r, fig.cap="Частота $MtrBTN1$ убывает по мере приблежения к источникам антропогенного загрязнения", fig.width=8}
...
```{r, fig.cap="Частота $MtrBTN1$ увеличивается на участках открытых для прибоя", fig.width=8}
...
```{r, fig.cap="Частота BTN1 увеличивается на участках с высокой долей молоди", fig.width=8}
sm %>% 
  ...
  theme_bw()+
  theme(axis.title.x = element_text(size = 15)) +
  annotate(geom = "text", x = -0.45, y = -2, label = "Много молоди", size = 5) +
  annotate(geom = "text", x = 0.08, y = -2, label = "Много старых", size = 5)  +
  annotate(geom = "segment", y = -2, yend = -2, x = -0.36, xend = -0.01, arrow = arrow(ends = "both", type = "closed", angle = 10), color = "red")
...
1. $MtrBTN1$ чаще встречается в поселениях мидий в наиболее благоприятных для хозяина условиях: вдали от источников антропогенного загрязнения, в местах с повышенной прибойностью, где идет активное пополнение мидий.
2. Значимой связи $MtrBTN2$ с изученными предикторами не выявлено, но эта линия встречается с той же частотой, что и $MtrBTN1$.
3. $MtrBTN2$, вероятно, более эврибионтная линия, имеет более широкую экологическую нишу, чем $MtrBTN1$. 

<!-- 2. *Альтернативное объяснение*: $MtrBTN1$ может быть обнаружен только в тех условиях, где хозяин имеет максимальные шансы выжить. -->
...
```{r, fig.cap= "При заражении $MtrBTN1$ видимые нарушения наступают на поздних стадиях заболевания"}
...
```{r, fig.cap= "При заражении $MtrBTN2$ нарушения наступают раньше, начиная с ранних стадий заболевания"}
...
- Предикторы: Размер последнего кольца, статус ($MtrBTN1$, $MtrBTN2$, Здоровые)
...
```{r, fig.cap="Прирост последнего года у зараженных BTN2 оказывается выше, чем у зараженных $MtrBTN1$ и у здоровых особей, которые между собой не отличаются"}
Pl_Status
```
