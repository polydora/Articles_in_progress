...
```{r}
# cover_23 <- read_excel("Data/Magadan_2021_2023_ecology_cleaned.xlsx", sheet = "Покрытия миидий 2023")
# 
# mean_cover_23 <-
#   cover_23 %>%
#   group_by(Site) %>%
...
Sites <- 
pca_scores_scam %>% arrange(PC1) %>% pull(Site) %>% unique() 

size$Site <- factor(size$Site, levels = Sites)
...
```{r, fig.cap="Cвязь плотности молоди со значениями PC1."}
ggplot(pca_scores_scam, aes(PC1, N_Juv)) + 
  geom_text(aes(label = Site)) +
  labs(x = "Значения PC1", y = "Плтность молоди") +
  geom_smooth(method = "lm") +
  theme_bw()
...
```{r, fig.cap="Cвязь общей плотности поселения со значениями PC1."}
ggplot(pca_scores_scam, aes(PC1, N_Total)) + 
  geom_text(aes(label = Site)) +
  labs(x = "Значения PC1", y = "Плотность поселения") +
  geom_smooth(method = "lm") +
  theme_bw()
...
cancer_2023_long <- 
cancer_2023 %>% 
  dplyr::select(Site, Sample, Salinity, Dist_Port, fetch, Sample, PC1, PC2, N_Large, N_Juv, N_Total, Mean_Cover, N, BTN1, BTN2) %>%
  # mutate(Sample = 1:nrow(.)) %>% 
  melt(., id.vars = c("Site", "Sample", "Salinity", "Dist_Port", "fetch",   "PC1", "PC2", "N_Large", "N_Juv", "N_Total",  "Mean_Cover", "N"), variable.name = "Lineage", value.name = "N_cancer") %>% 
...
#   filter(Lineage == "BTN1")
# 
# mod_foo <- lm(N_helthy ~ Dist_Port + fetch + PC1 + Mean_Cover + OGP, data = df)
# 
# library(car)
# vif(mod_foo)
...
Mod_btn1_btn2_no_re <- gam(cbind(N_cancer, N_helthy) ~ s((Dist_Port), by = Lineage, k = 10, bs = "tp") + s((fetch), by = Lineage, k = 10, bs = "tp")  + s((PC1), by = Lineage, k = 10, bs = "tp") + s((OGP), by = Lineage, k = 5, bs = "tp")  +  Lineage,  family = "binomial", method = "REML", data = cancer_2023_long )
...
sm %>% 
  ...
  # theme_bw()+
  # theme(axis.title.x = element_text(size = 15)) +
  # annotate(geom = "text", x = 0.45, y = -3.8, label = "Низкая \nплотность", size = 5) +
  # annotate(geom = "text", x = 100, y = -3.8, label = "Высокая \nплотность", size = 5)  +
...
## Проверка предсказаний модели на материалах 2021 года

Здесь все надо проверять!
...
OGP_21$Site <- factor(OGP_21$Site)
...
predictors_21 <-
  merge(OGP_21, points_21, all.y = TRUE )

predictors_21 <-
merge(predictors_21, pca_scores_scam)
...
cancer_2021_long <- 
cancer_2021 %>% 
  melt(., id.vars = c("Site", "N"), variable.name = "Lineage", value.name = "N_cancer") %>% 
  mutate(N_helthy = N - N_cancer)

cancer_2021_long$Site <- factor(cancer_2021_long$Site)


cancer_2021_long <- 
merge(cancer_2021_long, predictors_21)  
...
```{r}
cancer_2021_long$Predicted_Prevalence <- 
predict(Mod_btn1_btn2_no_re, newdata = cancer_2021_long, type = "response")
```
...
```{r}
cancer_2021_long %>% 
  ggplot(aes(Prop_BTN, Predicted_Prevalence)) +
  geom_point() +
  facet_wrap(~Lineage) +
  geom_smooth(method = "lm") +
  geom_abline()
...
myt_ind_clean$Rate_of_aneuploid_cells
...
## Связь размера с долей анеуплоидных клеток.
```{r}
myt_ind_clean %>%
  ggplot(aes(x = BTN_Type, y = L)) +
  geom_boxplot()
  
myt_ind_clean %>% 
  select(Rate_of_aneuploid_cells, L, BTN_Type) %>% 
melt(id.vars = c("Rate_of_aneuploid_cells", "BTN_Type")) -> 
  L_Rate_aneuploid


Mod_L_RAC <- gam(value ~ s(Rate_of_aneuploid_cells, by = BTN_Type) + BTN_Type, data = L_Rate_aneuploid %>% filter(BTN_Type != "Здоровые")) 
...
L_Rate_aneuploid <-
L_Rate_aneuploid %>% 
  mutate(Categor = BTN_Type) %>% 
  mutate(Categor = ifelse(BTN_Type == "Здоровые", "H", Categor ))

one_hot <- model.matrix(value ~ Categor - 1, data = L_Rate_aneuploid) %>% 
  as.data.frame()

L_Rate_aneuploid <- cbind(L_Rate_aneuploid, one_hot)

L_Rate_aneuploid  %>% 
  select(value, Categor1, Categor2) ->
  df
  
str(df)
dim(df)
  
L_Rate_aneuploid$BTN_Type_int <- as.integer(L_Rate_aneuploid$BTN_Type)-1
...
df <- 
  L_Rate_aneuploid %>%
  mutate(
    is_btn1 = as.integer(BTN_Type == "BTN1"),
    is_btn2 = as.integer(BTN_Type == "BTN2")
  )
...
# Мультиномиальная GAM модель
gam_multinom <- gam(list(
  is_btn1 ~ s(value, k = 15, bs = "cr"),  # Вероятность btn1 относительно h
  is_btn2 ~ s(value, k = 15, bs = "cr")   # Вероятность btn2 относительно h
), family = multinom(K = 2),  # K = число категорий минус 1
data = df,
method = "REML")
