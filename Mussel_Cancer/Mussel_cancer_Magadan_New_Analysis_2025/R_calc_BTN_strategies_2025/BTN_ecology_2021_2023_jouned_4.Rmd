...
В этом документе анализируются по отдельности две линии BTN2.1 и BTN2.2
...
cancer %>% 
  group_by(Site) %>% 
  summarise(Lat = mean(Lat), Lon = mean(Lon), N_total = sum(N), BTN1 = sum(BTN1), BTN2 = sum(BTN2), BTN2.1 = sum(BTN2.1), BTN2.2 = sum(BTN2.2)) %>% 
  mutate(BTN1 = BTN1/N_total*100, BTN2 = BTN2/N_total*100, BTN2.1 = BTN2.1/N_total*100, BTN2.2 = BTN2.2/N_total*100) %>% 
  dplyr::select(Site, Lat, Lon, BTN1, BTN2, BTN2.1, BTN2.2)  %>% 
...
Pl_map_BTN <- 
Pl_map + 
  geom_point(data = prop_BTN %>% filter(Lineage != "BTN2"), aes(x = Lon, y = Lat, size = Prop_BTN, group = 1), shape = 21, fill = "yellow") + 
  theme_map()+
  guides(size = "none") +
...
```{r}
Pl_hist <- 
  ggplot(prop_BTN %>% filter(Lineage != "BTN2"), aes(x = Prop_BTN)) +
...
Prob_BTN2.1_total <- sum(myt$BTN2.1)/sum(myt$N)
Prob_BTN2.2_total <- sum(myt$BTN2.2)/sum(myt$N)
...
```{r}
myt %>% 
  select(Site_code,  Year,  N, BTN1, BTN2.1, BTN2.2) %>%
  group_by(Site_code, Year) %>% 
  summarise(N = sum(N), BTN1 = sum(BTN1), BTN2.1 = sum(BTN2.1), BTN2.2 = sum(BTN2.2)) %>% 
...
```{r}
draw(Mod, residuals = T, select = 11)+ 
  geom_hline(yintercept = 0, linetype = 2)
```
...
```{r}
draw(Mod, residuals = T, select = 12)+ 
  geom_hline(yintercept = 0, linetype = 2)
```

Значимая связь с OGP пропала. 
