...
myt <-
myt %>% 
  filter_out(Year == 2023 & Site_code == "KHOL" & Sample %in% c("A", "B"))



# Суммарные описания сайтов

myt %>%
  group_by(Site_code, Year) %>%
  summarise(Total_N = sum(N), BTN1 = sum(BTN1)/Total_N * 100, BTN2 = sum(BTN2) / Total_N *100) %>%
  filter(BTN1 != 0 & BTN2 != 0) %>% 
  arrange(BTN2)
...
myt %>%
  group_by(Site_code, Year) %>%
  summarise(Total_N = sum(N), BTN1 = sum(BTN1)/Total_N, BTN2 = sum(BTN2) / Total_N) %>%
  arrange(BTN2) -> 
  df_summary_site

nrow(df_summary_site)

  cor.test(df_summary_site$BTN1, df_summary_site$BTN2, method = "spearman")
...
N_mussel_processed <- sum(myt$N)

N_mussel_infected <- sum(myt$BTN1) + sum(myt$BTN2)
...
# myt %>% 
#   group_by(Site_code, Year) %>% 
#   summarise(BTN1 = sum(BTN1), BTN2 = sum(BTN2), Total_N = sum(N)) %>% 
#   mutate(Prop_BTN1 = BTN1/Total_N, Prop_BTN2 = BTN2/Total_N) %>% 
#   arrange(desc(Prop_BTN1))
# 
# myt %>% 
#   group_by(Site_code, Year) %>% 
#   summarise(BTN1 = sum(BTN1), BTN2 = sum(BTN2), Total_N = sum(N)) %>% 
#   mutate(Prop_BTN1 = BTN1/Total_N, Prop_BTN2 = BTN2/Total_N) %>% 
#   arrange(desc(Prop_BTN2))
# 
...
Pl_map +
  geom_point(data = points_local, aes(x = lon, y = lat, group = 1, fill = Group), shape = 21,  size = 3, position = position_jitter(width = 0.01)) +
...
At each site: projective cover of mussels using photos, 1-3 qualitative samples of big mussels (L>25 mm) for BTN diagnostics by flow cytometry and for reproductive state by gonad smears under microscope (data on gonads is missing for some samples), 3-4 quantitative samples for mussel demography (local abundance, size structure) and growth rate by year rings on shells (only for old mussels). 
...
```{r, fig.cap = "**Fig. 3. Prevalence of BTN1 and BTN2 in samples. Points with zero prevalence are not shown.**", dpi = 600, fig.width= 8}
Pl_map_BTN
```
...
# OGP берем из файла ЕДИНСТВЕННЫЙ ФАЙЛ 20.07.2026_OGP_recalculated.xlsx, гдепеременная OGP_site_2 вычислена после переизмерений всех моллюсков, а также после исключения из вычислений OGP больных моллюсков. В расчетах учтены моллюски с возрастом не менее 2 лет и не более 10 лет (!!!).  Все вычисления лежат в коде "OGP_vs_Increment.R". То есть OGP_site_2 говорит о потенциях роста у здоровых моллюсков.  



growth <- read_excel("Data/ЕДИНСТВЕННЫЙ ФАЙЛ 20.07.2026_OGP_recalculated.xlsx", na = "NA")
...
# 
# growth_1$OGP_site[growth_1$Site_code == "KHOL" & growth_1$Year == 2023] <- 1.065483
# 
...
#   growth %>%
#   group_by(Site) %>%
#   summarise(OGP = mean(OGP_site_2)) #At! Это уже проверенные данные


ogp <-
  growth %>%
  group_by(Site) %>%
  summarise(OGP = mean(OGP)) #At! По просьбе ПП OGP_site будем считать как среднее значение индивидуального OGP


# ogp %>% 
#   ggplot(aes(OGP, OGP_Julia))+
#   geom_point() +
#   geom_abline()
...
# # Обобщенные описания изученных моллюсков
# 
# nrow(growth)
# range(growth$L, na.rm = T)
# median(growth$L, na.rm = T)
# 
# growth %>%
#   filter(BTN_genotype %in% c("BTN2.1", "BTN1", "BTN2.2")) %>%
#   nrow()
# 
# growth %>%
#   filter(BTN_genotype %in% c("BTN2.1", "BTN1", "BTN2.2")) %>%
...
```{r fig.cap="**Fig. 4. Ordination of population parameters (abundance of size classes, biomass and mussel cover) in PCA axes.**"}
...
PC1 explains `r round(sum_pca_scam$cont$importance[2,1]*100, 1)`

PC2 explains `r round(sum_pca_scam$cont$importance[2,2]*100, 1)`
...
pca_scores_scam %>% 
  # filter(Site %in% c("KHOL","MAM", "MAR", "NUK-I", "UM", "VES")) %>% 
  ggplot(aes(x = factor(Year), y = PC1, group = factor(Year) )) +
  geom_boxplot() +
  labs(x = "Year", y ="PC1") ->
  Pl_PC1_year
...
Pl_PC1_year +
  geom_point(data = pca_scores_scam %>% filter(Site %in% c("KHOL","MAM", "MAR", "NUK-I", "UM", "VES")), color = "red", size = 4) + 
  geom_line(data = pca_scores_scam %>% filter(Site %in% c("KHOL","MAM", "MAR", "NUK-I", "UM", "VES")), 
            aes(group = Site),  # та же группировка
            color = "gray", linewidth = 1, linetype = 2, alpha = 0.5)
...
```{r, fig.height=10, fig.cap="**Fig. 8. Population size structures (PC1 values in brackets).**"}
growth %>% 
  select(Site, Year, L) %>% 
...
Correlation between lineages prevalence
...
prop_BTN_2_points %>% 
  ...
  geom_line(data = prop_BTN_2_points %>% filter(Site %in% c("KHOL") & Lineage == "BTN2"), aes(group = Site), 
            ...
            linetype = 1,
            arrow = arrow(angle = 15, type = "closed", ends = "first")) +
  geom_point(data = prop_BTN_2_points %>% filter(Site %in% c("KHOL") & Lineage == "BTN2"), aes(color = as.factor(Year)), size = 3) +
...
```{r, fig.cap="**Fig. 13. Prevalence of BTN depending on mussel growth rate (OGP) in populations.**"}
...
To evaluate the association between multiple environmental and genomic predictors and the binary outcome of cell health status, we employed a Generalized Additive Mixed Model (GAMM) with a binomial error distribution and a logit link function. The response variable was specified as a two-column matrix of success–failure counts, representing the number of cancerous (N_cancer) and healthy (N_healthy) cells observed per sampling unit.

The fixed-effect structure included the categorical variable Lineage as a parametric term. For each continuous predictor, we fitted separate smooth spline functions for each level of Lineage using the by variable syntax, allowing the shape of the relationship to vary across cell lineages. All smooth terms were constructed using cubic regression splines with shrinkage (bs = "cs") and a basis dimension of k = 5, which provides sufficient flexibility to capture nonlinear patterns while limiting excessive complexity. The continuous predictors examined were Dist_Port, log-transformed fetch, the first two principal components (PC1 and PC2), OGP, and Salinity. To account for unmeasured temporal and spatial heterogeneity, we included random intercepts for Year and Site, specified as independent random effect smooths (bs = "re").

Model parameters were estimated using Restricted Maximum Likelihood (method = "REML") to obtain unbiased estimates of the variance components. To prevent overfitting and facilitate automatic predictor selection, we activated the double-penalty option (select = TRUE), which applies an additional shrinkage penalty capable of removing an entire smooth term from the model when its contribution is negligible. Furthermore, the smoothness selection was adjusted by setting the penalty multiplier to gamma = 2. This value was empirically determined during model tuning as the one that minimized the Bayesian Information Criterion (BIC), thereby ensuring a parsimonious final model with appropriately smoothed functional relationships. All analyses were performed using the mgcv package in R (version X.X) on the dataset cancer_21_23_predictors.
...
Mod <- gam(cbind(N_cancer, N_helthy) ~ 
           ...
           method = "REML", 
           select = TRUE,
           gamma = 2, #Этому значению соответствует минимальный BIC
...
<!-- sim_res <- simulateResiduals(fittedModel = Mod, plot = T, n = 2500, refit = FALSE) -->
...
Significant associations were found for BTN1 with distance to port, Fetch, and PC1, while no significant patterns were found for BTN2. In terms of growth rates, BTN2 was significantly associated with OGP (p=0.007), while the association for BTN1 was not significant.
...
```{r}
Pl_BTN2_OGP <- 
draw(Mod, residuals = T, select = 10)+ 
  geom_hline(yintercept = 0, linetype = 2)
```
...
myt_ind <- read_excel("Data/ЕДИНСТВЕННЫЙ ФАЙЛ 20.07.2026_OGP_recalculated.xlsx", na = "NA")
...
myt_ind <-
myt_ind %>%
  mutate(BTN_Type = case_when(BTN_genotype == "BTN2.2" ~ "BTN2",
...
# Удаляем идий, которые были испольованы для анализа роста, но не анализировались с для цитометрии или иного определения налияия DN
...
## Effects of BTN infection on gonad quality



```{r}
# Удаляем тех, у кого не смотрели мазки гонад
myt_ind_clean <-
  myt_ind %>%
...
myt_ind_clean$Gonad_quality <- factor(myt_ind_clean$Gonad_quality, labels = c("Gametes present", "No gametes"))

```



```{r, fig.cap=" **Fig. 18. Frequency of mussels with gametes in gonad smears and without gametes among infected with different BTN lineages, compared to healthy specimens.**",  fig.width=10, eval=TRUE}

# At! размер картинки на слайдах этого типа регулируется fig.width=

myt_ind_clean %>% 
  group_by(Year, BTN_Type, Gonad_quality) %>%  
  summarise(Frequency = n(), Prop_aneupl = median(Rate_of_aneuploid_cells), .groups = 'drop') ->
  prop_aneupl



# myt_ind_clean %>%
#   group_by( BTN_Type, Gonad_quality) %>%  # Группируем по годам и типам BTN
#   summarise(Frequency = n(), .groups = 'drop') %>%  # Считаем количество наблюдений
#   group_by( BTN_Type) %>%  # Группируем для вычисления долей
#   mutate(Frequency = Frequency / sum(Frequency)) %>% 
#   dcast(BTN_Type ~ Gonad_quality  )
...
myt_ind_clean %>%
  ...
  theme_void() +
  facet_wrap( ~ BTN_Type) + # Разделяем на подграфики по годам и типам BTN
...
myt_ind_clean %>%
  mutate(Gonad_Out = ifelse(Gonad_quality == "No gametes", 1, 0)) %>%
  filter(BTN_Type != "Healthy") ->
...
sm %>%
  ...
  labs(x = "Proportion (%) of aneuploid cells", y = "Frequency of gamete-free specimens") +
  # ylim(-5, 4)+
...
myt_ind_clean_not_healthy %>% 
  ...
  summarise(Rate_of_aneuploid_cells = mean(Rate_of_aneuploid_cells), Prop_anomaly = mean(Gonad_quality == "No gametes"), n = n())
...
```{r, fig.width=15, fig.height=9, fig.cap="**Fig. 19.Dependence of reproductive states on the proportion of aneuploid cells.**"}
...
myt_ind_clean_not_healthy %>% 
  # filter(Gonad_quality == "No gametes") %>% 
  ggplot(aes(x = Gonad_quality, y = Rate_of_aneuploid_cells)) +
  geom_violin(aes(fill = Lineage )) +
  geom_boxplot(width = 0.1) +
  scale_fill_manual(values = c("red", "blue")) +
  facet_wrap(~Lineage) +
  labs(y = "Proportion of anneuploid cells", x = "Reproductive state") +
  coord_flip()
```


The figure above illustrates that the negative effect of the BTN2 lineage on reproductive state manifests at slightly earlier stages of disease progression than that of BTN1. 



## Analyzis of acini size
...
gon <-
gon %>% 
  rename(BTN_Type = BTN_class_1_or_2)


gon$BTN_Type <- factor(gon$BTN_Type)


gon_btn <- 
  gon %>% 
  filter_out(BTN_Type == "no")
...
mod_gon <- glm(cbind(Gonad_grid,Tishue_grid) ~ BTN_Type*Aneuploidy_rate, data = gon_btn, family = "binomial")
...
predict_function <- function(data, indices, newdata_grid) {
  ...
  boot_mod <- tryCatch({
    glm(cbind(Gonad_grid, Tishue_grid) ~ BTN_Type * Aneuploidy_rate,
        data = boot_data,
        family = "binomial")
...
btn_levels <- unique(gon_btn$BTN_Type)
...
# Запускаем бутстреп
set.seed(123456)  # для воспроизводимости
...
newdata$predicted <- predict(mod_gon, newdata = newdata, type = "response")
...
Pl_gonad_btn <- 
ggplot(newdata, aes(x = Aneuploidy_rate, y = predicted, color = BTN_Type)) +
  geom_line(linewidth = 2) +
  geom_ribbon(aes(ymin = conf_low, ymax = conf_high, fill = BTN_Type), 
              alpha = 0.2, color = NA, fill = "gray40") +
...
Pl_gonad_btn +
  annotate(geom = "star", x = 0, y = mean_value, 
           size = 5, fill = "black", color = "black") +
  ...
  labs(y = "Proportion of germinal tissue", x = "Proportion of aneuploid cells (%)")
...
mod_gon_summary <- tidy(mod_gon)

ft <- flextable(mod_gon_summary)
...
Было выявлено значимое взаимодействие предикторов, что говори о разной связи между относительным размером ацинусов и степенью развития болезни у двуз линий BTN. На первых этапах заболевания BTN1 относительный размер ацинусов еще близок к таковому у здоровых моллюсков. Значительные отличия наблюдаются только при доле анеуплоидных клеток более 25%. У BTN2уже с первых этапов развития болезни относительный размер ацинусов значительно меньше, чем у здоровых особей. 
...
# Определяем границы варьирования размеров больных мидий
...
# Отфильтровываем мидий, у которых размеры укладываются в размеры больных мидий

myt_ind_size_filtered <- 
  myt_ind %>% 
  filter(L <= BTN_L_range[2] & L >= BTN_L_range[1]) 




myt_ind_size_filtered$BTN_Type <- factor(myt_ind_size_filtered$BTN_Type, levels = c("Healthy", "BTN1", "BTN2"))
...
```{r, echo=TRUE, eval=FALSE}
Mod_prop_incr_btn <- gam(Fi_Increment ~ s(OGP, by = BTN_Type) + s(Last_ring, by = BTN_Type) + BTN_Type + s(Site_code, bs = "re"), data = myt_ind_size_filtered)
...
myt_ind_size_filtered$Site_code <- factor(myt_ind_size_filtered$Site_code)
...
mod_sum4 <- tidy(Mod_prop_incr_btn, parametric = F)
...
My_data <- data.frame(BTN_Type = levels(myt_ind_size_filtered$BTN_Type), Last_ring = mean(myt_ind_size_filtered$Last_ring), OGP = mean(myt_ind_size_filtered$OGP))



Predicted <- predict(Mod_prop_incr_btn, newdata = My_data, exclude = "s(Site_code)", newdata.guaranteed=TRUE, se.fit = TRUE)
...
My_data$BTN_Type <- factor(My_data$BTN_Type, levels = c("Healthy", "BTN2", "BTN1"))
...
Pl_Status <-
  ...
  geom_signif(comparisons = list(
      c("BTN1", "BTN2"),
      c("BTN2", "Healthy")),
      annotations = c("W = 23.6, p <0.001", "W = 19.1, p < 0.001")
      )  +
  # theme_bw()+
...
myt_ind_size_filtered$BTN_Type <- factor(myt_ind_size_filtered$BTN_Type, levels = c("Healthy", "BTN2", "BTN1"))

Pl_Staus_initial <-
  ggplot(data = myt_ind_size_filtered, aes(x = BTN_Type, y = Prop_Increment)) +
  # geom_violin(aes(fill = BTN_Type)) +
  geom_boxplot(aes(group = BTN_Type, fill = BTN_Type), outliers = F) +
  # facet_wrap(~ Gonad_quality) +
  scale_fill_manual(values = c( "green", "blue", "red")) +
  labs(x = "", y = "Last year's relative \ngrowth increment") +
   guides(fill = "none") +
  geom_point(position = position_jitter(width = 0.08), color = "gray", alpha = 0.5)
  


# Pl_L_initial <-
#   ggplot(data = myt_ind_size_filtered, aes(x = BTN_Type, y = L)) +
#   geom_violin(aes(fill = BTN_Type)) +
#   # geom_boxplot(aes(group = BTN_Type)) +
#   # facet_wrap(~ Gonad_quality) +
#   scale_fill_manual(values = c( "green", "blue", "red")) +
#   labs(x = "", y = "Shell length") +
#    guides(fill = "none") +
#   geom_point(position = position_jitter(width = 0.08), color = "gray", alpha = 0.5)
...
Pl_OGP_initial <-
  ggplot(data = myt_ind_size_filtered, aes(x = BTN_Type, y = OGP)) +
  geom_boxplot(aes(fill = BTN_Type, group = BTN_Type), outliers = F) +
  # geom_boxplot(aes(group = BTN_Type)) +
  # facet_wrap(~ Gonad_quality) +
  scale_fill_manual(values = c( "green", "blue", "red")) +
  labs(x = "", y = "Individual OGP") +
   guides(fill = "none") +
  geom_point(position = position_jitter(width = 0.08), color = "gray", alpha = 0.5)
...
plot_grid(Pl_Staus_initial, Pl_OGP_initial,  Pl_Status,  labels = c("A", "B", "C"))


library(patchwork)

(Pl_OGP_initial + Pl_Staus_initial) / 
  (Pl_Status ) +
...
## Addition model for dependence relative growth on proportion of anneuploid cells
...
myt_ind_size_filtered %>% 
  filter(Rate_of_aneuploid_cells >0) ->
  myt_ind_size_filtered_not_healthy

myt_ind_size_filtered_not_healthy$BTN_Type <- factor(myt_ind_size_filtered_not_healthy$BTN_Type)




Mod_prop_incr_btn_aneuploid_rate <- 
  gam(Fi_Increment ~   
        s(OGP, by = BTN_Type) + 
        s(Last_ring, by = BTN_Type) +
        s(Rate_of_aneuploid_cells,  by = BTN_Type) +
        BTN_Type + 
        s(Site_code, bs = "re"),
      method = "REML",
      select = TRUE,
      gamma = 0.5,
      data = myt_ind_size_filtered_not_healthy)

summary(Mod_prop_incr_btn_aneuploid_rate)

draw(Mod_prop_incr_btn_aneuploid_rate, select = 5:6, residuals = T,scales = "fixed" )
```


```{r}
myt_ind_size_filtered_not_healthy %>% 
  mutate(Rel_Increment = Fi_back(Fi_Increment)) %>% 
  ggplot(aes(x = Rate_of_aneuploid_cells, y = Rel_Increment, color = BTN_Type)) +
  geom_point() +
  geom_smooth(method = "lm") +
  scale_color_manual(values = c("blue", "red")) +
  labs(x = "Rate of aneuploid cells", y = "Relatove increment", title = "Raw Data")

```


There ia no dependency of relative increment on Rate of anneuploid cells

## Addition analysis of dependencies between biotic parameters of mussels settlementa and environmental predictors

```{r}
prop_BTN_2_points %>% 
  filter(Lineage == "BTN2") %>% 
  select(Biomass, Cover, OGP, L13, L13,  L18, L23,  L28, L3, L33, L38, L43, L48, L53, L58, L8) ->
  mussel_biotic_parameters


prop_BTN_2_points %>% 
  filter(Lineage == "BTN2") %>% 
  select(Year, Salinity, Dist_Port, fetch) ->
  external_predictors


external_predictors$Year <- factor(external_predictors$Year)


mod_cca <- cca(mussel_biotic_parameters ~ Year + Salinity + Dist_Port + fetch, data = external_predictors)
...
plot(mod_cca, display = c("cn", "species"))
