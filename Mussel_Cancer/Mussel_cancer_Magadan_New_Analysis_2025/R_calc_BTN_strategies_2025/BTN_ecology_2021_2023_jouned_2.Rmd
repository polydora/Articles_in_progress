...
```{r}
df %>% 
  ggplot(aes(x = Mean_Cover, Biomass )) +
  geom_point() +
  facet_wrap(~Year)
```
...
pca_scores_scam %>% 
  ggplot(aes(PC1, N_Large/N_Total)) +
  geom_point() +
  geom_text_repel(aes(label = Site_Year)) +
  labs(y = "Proportion of Adult")
...
```{r}
Sites_Year <- 
pca_scores_scam %>% arrange(Site) %>% pull(Site_Year) %>% unique() 
