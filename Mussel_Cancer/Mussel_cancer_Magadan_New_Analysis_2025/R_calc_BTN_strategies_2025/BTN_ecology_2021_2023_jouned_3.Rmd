...
Areas surveyed only in 2021 are marked in white, only in 2023 in yellow, and in both years in red. 
...
```{r fig.cap="Ordination of popukation parameters in PCA axes."}
...
Thus PC1 reflecs the temporal changes in mussel population structure: аfrom old-dominated settlements to young-dominated ones. 
...
То есть, в среднем, на шести переописанных банках частота BTN1 в период с 2021 по 2023 гг. уменьшилась. Явного временного патерна для BTN2 нет.
...
BTN1_PC1 +
  geom_point(data = df_21_23_btn1, color = "gray", size = 4) +
  geom_smooth(data = prop_BTN_2_points %>% filter(Lineage == "BTN1"), aes(x = PC1, y = Prop_BTN))
```
...
Continuous predictors (distance to port, fetch, PC1, and 
OGP) were modeled as smooth terms using cubic regression splines 
with shrinkage (bs = 'cs', k = 5). Lineage was included as a fixed factor, and 
