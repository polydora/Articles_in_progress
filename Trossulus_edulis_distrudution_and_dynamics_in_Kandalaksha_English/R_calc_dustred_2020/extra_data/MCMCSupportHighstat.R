...
nchains <- 3
AllID <- rep(1:nchains, each = nrow(mat1))  #groups id
PE <- posterior.estimates[,vars]  #Selected variables

ID        <- colnames(PE)
AllY      <- as.vector(PE)
AllX      <- rep(rep(1:nrow(mat1), 3), length(vars))
AllPanels <- rep(ID, each=nrow(PE))
 
Z <- xyplot(AllY ~ AllX | factor(AllPanels) ,
       type = "l",
       strip = strip.custom(bg = 'white',
       par.strip.text = list(cex = 1.2)),
       scales = list(x = list(relation = "same", draw = TRUE),
...
#for each variable 
  ...
  for (i in vars){
	...
	id          <- rep(rep(i, length = NumLags),NumBerChains)
...
PE <- posterior.estimates[,vars]  #Selected variables
  ...
  for (i in vars){
    ...
    acf.Vari <- Lag.Vari <- NULL
    for (j in 1:NumBerChains){
      acf.Varij <- acf(x1[AllID==j], plot = FALSE)
      acf.Vari <- c(acf.Vari, acf.Varij$acf)
      Lag.Vari <- c(Lag.Vari, acf.Varij$lag)
    }
    NumLags <- length(acf.Varij$lag)
...
##############################################################
MyCodaHist <- function(xx, vars){
#Small function to make an histogram per chain,
#for each variable 
  #xx <- J1
  mat1 <- as.matrix(xx[[1]])
  mat2 <- as.matrix(xx[[2]])
...
Z <- histogram( ~ AllParams | factor(AllID),
           ...
           panel = function(x, ...) {
             ...
             a <- quantile(x, prob=c(0.025, 0.975))
             panel.abline(v = a[1], lwd = 1, col =1, lty =2)
             panel.abline(v = a[2], lwd = 1, col =1, lty =2)
             })
  print(Z)
}
...
##################################################
MyCodaNumOutput <- function(xx,vars){
#Present numerical output
    mat1 <- as.matrix(xx[[1]])
    mat2 <- as.matrix(xx[[2]])
    mat3 <- as.matrix(xx[[3]])
    PE <- rbind(mat1, mat2, mat3)
    OUT <- matrix(nrow = length(vars), ncol=4) 
    j<-1
