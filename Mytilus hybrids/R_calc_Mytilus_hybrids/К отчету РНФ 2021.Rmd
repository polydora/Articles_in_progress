...
$$
md =1- \frac{\sum(P_i - \overline P)^2}{Ptros(1-Ptros)}
$$
...
Pl_Sal_index +theme(panel.grid = element_blank(), legend.background = element_blank(), legend.position = c(0.8, 0.2)) + ggtitle(TeX("Salinity index = $\\sqrt{rank(Distance) \\times  rank(Drainage^{-1})}$")) + theme(plot.title = element_text(hjust = 0.5, size = 10))
...
```{r}
Mod5 <- gam(md_1 ~ s(Ptros) + s(Sal_index), data = hybr_pop_riv , family = "betar")


Mod6 <- gam(P_hybr ~ s(Ptros) + s(Sal_index),  data = hybr_pop_riv , family = "betar"  )
...
ggplot(My_data, aes(x = Ptros, y = Predicted, group = Sal_index)) + geom_line() + scale_color_gradient(low = "yellow", high = "red") + geom_point(data = hybr_pop_riv, aes(y = md_1, fill = Sal_index, shape = Region), shape = 21, color = "black", size = 4) + scale_fill_gradient(low = "white", high = "darkblue")  + facet_wrap(~Region) + labs(y = TeX("md = $1- \\frac{\\sum(P_i - \\overline P)^2}{Ptros(1-Ptros)$}"))
```




```{r fig.cap="Рисунок +. Связь доли гибридов (A) и показателя генетической смешанности (B) с индексом солености, согласно модели GAM1 и GAM2, соответственно. Приведеные линии регрессии отражают частные эффекты предиктора (влияние индекса соолености при учете влияния сглаживающей функции). "}

library(gratia)


Pl_mod6 <- draw(Mod6)[[2]] + ggtitle("A") + labs(x = "Индекс солености", y = "Частный эффект (Phybr)")  
Pl_mod5 <- draw(Mod5)[[2]] + ggtitle("B") + labs(x = "Индекс солености", y = "Частный эффект (md)")

library(cowplot)
plot_grid(Pl_mod6, Pl_mod5)

```



Обе модели выявляют статистически значимую связь зависимых переменных с индексом солености (Таблица ++ и ++). При этом, согласно обеим моделям, по мере увеличения индекса солености степень гибридизации падает будь она оценена в  терминах доли гибридов (отрицательный коэффициент при Phybr, рис. ++, A) или  в терминах индекса смешанности (положительный коэффициент при md, рис. ++, B). То есть в участках, подвергающихся хроническому опреснению, степень гибридизации между *M.edulis* и *M.trossulus*, выше, чем на участках, удаленных от мощных источников опреснения.  
...
```{r}
library(broom)
kable(tidy(Mod6), caption = "Таблица ++. Результаты регрессионного анализа $Phybr$ в зависимости от от Ptros и индекса солености (модель GAM1).")
...
kable(tidy(Mod5), caption = "Таблица ++. Результаты регрессионного анализа $md$ в зависимости от Ptros и индекса солености (модель GAM2).")
