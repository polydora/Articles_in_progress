...
## Анализ связи степени генетическойсмешанности с соленостью.
...
ggplot(My_data, aes(x = Ptros, y = Predicted, group = Sal_index)) + geom_line() + scale_color_gradient(low = "yellow", high = "red") + geom_point(data = hybr_pop_riv, aes(y = md_1, fill = Sal_index, shape = Region), shape = 21, color = "black", size = 4) + scale_fill_gradient(low = "yellow", high = "red")  + facet_wrap(~Region) + labs(y = "1-md")
```
...
ggplot(My_data, aes(x = Ptros, y = Predicted, group = Sal_index)) + geom_line() + geom_point(data = hybr_pop_riv, aes(y = P_hybr, fill = Sal_index, shape = Region), shape = 21, color = "black", size = 4) + scale_fill_gradient(low = "yellow", high = "red")  + facet_wrap(~Region) + labs(y = "Доля гибридов в популяции (P_hybr)")
```
...
<!-- ```{r} -->
<!-- My_data_2 <- expand.grid(Ptros = seq(0,1,0.01), Sal_index = mean(hybr_pop_riv$Sal_index), max(hybr_pop_riv$Sal_index), 0.1 ) -->


<!-- My_data_2$Predicted <- predict(Mod5, newdata = My_data_2, type = "response") -->

<!-- ggplot(My_data_2, aes(x = Ptros, y = Sal_index, color = Predicted)) + geom_tile() +   geom_point(data = hybr_pop_riv, aes(x = Ptros, y = Sal_index, fill = md_1), shape = 21, color = "black", size = 4) + scale_fill_gradient(low = "gray20", high = "white") +  scale_color_gradient(low = "gray20", high = "white") -->
...
Pl_sm <- plot(sm(b, 1)) +  l_fitLine(colour = "red")  +l_ciLine(mul = 5, colour = "blue", linetype = 2) + l_points(shape = 19, size = 2) + theme_bw()


Pl_term <- plot(pterm(b, 1)) + l_ciPoly() + l_fitLine() + l_points(shape = 19, size = 2)+ theme_bw()


library(cowplot)
gridPrint(Pl_sm, Pl_term, ncol = 2)


gridPrint(plot(sm(b, 1)) + labs(title = NULL) + l_points(),
          plot(pterm(b, 1)) + l_ciPoly() + l_fitLine() + l_points(), ncol = 2)
...
sites.environment <- data.frame(Lon = hybr_pop_riv$lon, Lat = hybr_pop_riv$lat, Sal = extract(salinity,hybr_pop_riv[,3:2]) )

qplot(hybr_pop_riv$Salinity, y = sites.environment$Sal) + geom_boxplot()
