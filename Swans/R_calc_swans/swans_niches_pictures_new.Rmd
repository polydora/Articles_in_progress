...
Pl_ice_free_period <-
  plot_grid(Pl_sp1, Pl_sp2, Pl_sp3, ncol = 1)



# 
# 
# Pl_ice_free_period_labeled <- ggdraw(Pl_ice_free_period) +
#   # Подпись для оси X (внизу)
#   draw_label("", 
#              x = 0.5, y = 0.1, vjust = -8, size = 12) +
#   # Подпись для оси Y (слева)
#   draw_label("Depth class", 
#              x = 0.01, y = 0.5, angle = 90, vjust = 0.5, size = 12)

# Отображаем результат
# Pl_ice_free_period_labeled
...
<!-- Это старая версия картинки. Надо подумать, какая лучше смотрится. -->
...
Pl_ice_period <- plot_grid(Pl_sp1, Pl_sp2, Pl_sp3, nrow = 1)


Pl_ice_period_labeled <- ggdraw(Pl_ice_period) +
  # Подпись для оси X (внизу)
  draw_label("Distance class", 
             x = 0.5, y = 0.1, vjust = 0, size = 12) +
  # Подпись для оси Y (слева)
  draw_label("Depth class", 
             x = 0.01, y = 0.5, angle = 90, vjust = 0.5, size = 12)

# Отображаем результат
# Pl_ice_period_labeled
...
(Pl_ice_free_period_labeled / Pl_ice_period_labeled) +
...
```{r}
plot_grid(Pl_Cygnets_Young, Pl_Cygnets_Old)
```



Это те же самые катринки но в другой модификации 

```{r}
draw(Mod_2, select = 3:4)
```




Вот Таблица summary подобранной модели, ее опять же предлагаю поместить в электронное приложение
...
ft %>%
  ...
  colformat_double(j = 5, digits = 3) %>% 
  set_header_labels(values = c("Smoother term", "edf", "ref.df", "F", "p-value" )) %>% 
  set_caption("Таблица 1. Результаты подгонки обобщенной аддитивной модели  (GAMM) для оценки влияния факторов на пространственное распределение выводков лебедя-шипуна")
...
Это та же катринка но в другой модификации 
...
ft %>%
  ...
  colformat_double(j = 5, digits = 3) %>% 
  set_header_labels(values = c("Smoother term", "edf", "ref.df", "F", "p-value" )) %>% 
  set_caption("Таблица 1. Результаты подгонки обобщенной аддитивной модели  (GAMM) для оценки влияния факторов на пространственное распределение линяющих особей лебедя-шипуна")
