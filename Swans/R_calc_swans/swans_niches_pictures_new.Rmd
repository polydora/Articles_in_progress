...
theme_set(theme(axis.text = element_text(size = 20), plot.title = element_text(size = 20, hjust = 0), plot.subtitle = element_text(size = 20, face = "italic"), axis.title = element_text(size = 20)))
...
ggplot(smooth_data_filtered, aes(Dist, Depth)) +
  ...
  coord_fixed() +
  ggtitle(label = "B", subtitle = "C.olor") +
  ...
  theme(
    ...
    legend.background = element_rect(fill = alpha("white", 0.5), color = "black"),
     legend.key.size = unit(0.5, "cm"),  # Размер ключей легенды
    legend.key.width = unit(0.5, "cm"), # Ширина ключей
    legend.text = element_text(size = 9), # Размер текста
    legend.title = element_text(size = 10) # Размер заголовка
    )
...
ggplot(smooth_data_filtered, aes(Dist, Depth)) +
  ...
  scale_fill_gradient2(low = color_low, mid = "white", high =  color_high) +
  coord_fixed() +
  ggtitle(label = " ", subtitle = "C.cygnus") +
  labs(fill = "Partial effect", x = NULL, y = NULL) +
 theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "gray90", color = NA),
...
Pl_sp3 <-
  ...
  scale_fill_gradient2(low = color_low, mid = "white", high =  color_high) +
  coord_fixed() +
  ggtitle(label = " ", subtitle = "C.bewickii") +
  labs(fill = "Partial effect", x = "Distance", y = NULL) +
 theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "gray90", color = NA),
    legend.position = c(0.86, 0.21),
...
ggplot(smooth_data_filtered, aes(Dist, Depth)) +
  ...
  scale_fill_gradient2(low = color_low, mid = "white", high = color_high) +
  coord_fixed() +
  ggtitle(label = "A", subtitle ="C.olor") +
  labs(fill = "Partial effect", x = NULL, y = "Depth") +
 theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "gray90", color = NA),
...
ggplot(smooth_data_filtered, aes(Dist, Depth)) +
  ...
  coord_fixed() +
  ggtitle(label = " ", subtitle ="C.bewickii") +
  labs(fill = "Partial effect", x = "Distance", y = "Depth") +
  theme(
    panel.grid = element_blank(),
    panel.background = element_rect(fill = "gray90", color = NA),
    legend.position = c(0.88, 0.18),
...
```{r, fig.cap="Рисунок ++. Соотношение численности особей трех видов лебедей, держащихся в разных типах разводий в период таяния льда. The ratio of the abundance of three swan species staying in different types of ice openings during the ice melting period. "}
combined_plots
```
...
ggplot(smooth_data_filtered, aes(Dist, Depth)) +
  ...
  theme(
    ...
    legend.background = element_rect(fill = alpha("white", 0.5), color = "black"))  +
  labs(fill = "Partial effect", x = "", y = "Depth")
...
ggsave(plot = Pl_Molting_Birds, filename = "Figures/Pl_Molting_Birds.png")


```


<!-- ```{r, fig.cap="Рисунок ++.  Связь обилия линяющих особей лебедя-шипуна с расстоянием от берега и глубиной в соответствии с моделью GAMM3. Relationship between the abundance of molting mute swan individuals and distance from shore and depth according to the GAMM3 model."} -->
...
combined_cygnets_molting <- image_read(c(
  "Figures/Pl_Cygnets_Young.png",
  "Figures/Pl_Cygnets_Old.png",
  "Figures/Pl_Molting_Birds.png")) %>%
  image_trim() %>%  # убирает пустые поля вокруг каждого изображения
  image_border("white", "0x10") %>%  # опционально: добавить небольшой отступ между ними
  image_append(stack = TRUE) 

# Сохранение
image_write(combined_cygnets_molting, "Figures/combined_cygnets_molting.png")

```



```{r, fig.cap = "Рисунок ++. Связь с расстоянием от берега и глубиной обилия выводков лебедя-шипуна, в соответствии с моделью GAMM2, и обилия линяющих особей, в соответствии с моделью GAMM3. Relationship between the distance from shore and depth and the abundance of mute swan broods (according to the GAMM2 model) and the abundance of molting birds (according to the GAMM3 model)." }

include_graphics("Figures/combined_cygnets_molting.png")
