...
cam_full <- read.table("Data/kambala_long_term_dynamics.csv", sep = ",", header = T, dec = ".")
...
left_prop <- cam %>% group_by(Bay, Year) %>% summarise(Left_prop = mean(Orient == "Left"), n = n()) %>% as.data.frame()

year_exclude <- c(2006, 2010)

# left_prop %>% group_by(Bay ) %>% summarise(mean(Left_prop), Sd = sd(Left_prop))

cam <- cam %>% filter(!(Bay == "Onega" &  (Year %in% year_exclude)) )
```
...
historic_data <- data.frame(Bay = c("Onega", "Onega", "Кandalaksha", "Кandalaksha"), Year = c(1950, 1948,  1962, 2006), Left_prop = c(0.285, 0.313,  0.38, 0.36), N = c(358, 182,  507, 450), Source = c(2, 1, 3, 4), shift = c(1.5, rep(-1.5, 3)) )
...
ggplot(mydata, aes(x = Year, y = Fit, group = Bay)) + geom_line(aes(linetype = Bay), size = 1) + 
  ...
  guides(shape = guide_legend(override.aes = list(size = c(3, 3, 4, 4) ))) +
  theme(axis.title = element_text(size = 15), legend.text = element_text(size = 15), legend.title = element_text(size = 15), axis.text = element_text(size = 12))


  # ggsave("fig_long-term.tiff", Pl)
...
```{r}
cam %>% group_by(Bay) %>% summarise(N = n())

cam %>% group_by(Bay, Year) %>% summarize(N = n(), Left_prop = mean(Orient2)) %>% as.data.frame()

cam %>% group_by(Bay, Year) %>% summarize(N = n(), Left_prop = mean(Orient2)) %>% as.data.frame() %>% group_by(Bay) %>% summarise(min = min(Left_prop), max = max(Left_prop), mean = mean(Left_prop), SD = sd(Left_prop))
