...
```{r}
kable(tidy(Mod_7), caption = "Model parameters")
```
...
new_data$Bay <- factor(new_data$Bay, levels = c("Onega Bay", "Dvina Bay", "Kandalaksha Bay", "Mezen Bay"))

predicted <- predict(Mod_7, newdata = new_data, se.fit = T, type = "response")

new_data$Predicted <- predicted$fit
new_data$SE <- predicted$se.fit


ggplot(new_data, aes(x = Bay, y = Predicted)) + geom_col(fill = "gray", color = "gray10") + geom_errorbar(aes(ymin = Predicted - 1.96*SE, ymax = Predicted + 1.96*SE), width = 0.1)  + labs(y = "Probability of left-sided morph")
...
```{r}
kable(tidy(M), caption = "Age Model 2 parameters")
...
# ggsave(filename = "Model2_Visualisation.tiff", plot = Pl_age_onega, dpi = 600)
