...
ggplot(new_data, aes(x = L, y = Predicted)) + 
  ...
  labs(x = "Length, cm", y = "Frequency of \nleft-sided morph") +
  geom_segment(data = kam_oneg, aes(x = L, xend = L, y = 0, yend = 0.01)) +
  theme(strip.text = element_blank()) + 
  ggtitle("A")+ theme(panel.grid = element_blank())
...
Pl_age_onega <- 
ggplot(mydata, aes(x = Age, y = fit, group = Sex))  + 
  geom_ribbon(aes(ymin = fit - 1.96*SE, ymax = fit + 1.96*SE), alpha = 0.3) + 
  geom_line(aes(linetype = Sex), size = 1) +
  geom_point(data = P_age, aes(x = Age, y = P_left, fill = Sex), size = 4, shape = 21) +
  theme_bw() +
  labs(y = "Frequency of \nleft-sided morph", x = "Age, years") + 
  scale_fill_manual(values = c("white", "black"))+
  theme(legend.position = "bottom") + 
  ggtitle("B") + theme(panel.grid = element_blank())

```


```{r fig.width=4, fig.cap="**Fig.** Association between probability of left-sided morph and fish length (A) and age (B). Points represent frequency of reversed fish in 25 length classes of equal sample size. Gray area around regression line represent 95% confidential interval."}


plot_grid(Pl_size_onega, Pl_age_onega, ncol = 1, rel_heights = c(1, 1.2))
