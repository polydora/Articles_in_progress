...
new_data <- kam %>% group_by(Bay) %>% do(data.frame(L = seq(min(.$L), max(.$L), length.out = 100) ))
...
Left_prop <- kam %>% group_by(Bay) %>% do(data.frame(L = .$L, Orient = .$Orient, Size_class = ntile(x = .$L, n = 10))) %>% group_by(Bay,Size_class) %>% summarise(L_mean = mean(L), Left_prop = mean(Orient == "Left"))
