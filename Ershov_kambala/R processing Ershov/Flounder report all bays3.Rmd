...
```{r }
ggplot(kam, aes(x = Bay, y = L,  fill = Orient)) + geom_boxplot(notch = T) + facet_wrap(~Sex) + scale_fill_manual(values = c("gray50", "gray90")) + theme(panel.grid = element_blank(), axis.text.x = element_text(angle = 30, vjust = 0.6))
```
...
frq_Orient <- kam %>% group_by(Bay) %>% summarise(Left = sum(Orient == "Left"), Right = sum(Orient == "Right"))
  
# Добавляем то, что не было измерено в Кандалакшском заливе
frq_Orient[frq_Orient$Bay == "Kandalaksha_Bay", 2:3] <- frq_Orient[frq_Orient$Bay == "Kandalaksha_Bay", 2:3] + c(37, 132)
...
kam %>% group_by(Bay) %>% summarise(Left_prop = mean(Sidedness == "Left"), n = n())
```
