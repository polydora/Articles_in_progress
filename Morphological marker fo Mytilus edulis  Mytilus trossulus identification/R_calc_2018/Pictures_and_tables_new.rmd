...
Table ++. Characteristics of modelling data set.
...
Model4: $logit(P_{correct}) = Morph + Ptros + Subset + Morph:Ptros + Morph:Subset + Ptros:Subset + Morph:Ptros:Subset$
...
*M.edulis* and *W* where used as basic levels for categorical predictors in all models.
...
Pl_mod2_with_initial_data <- Pl_mod2_with_initial_data +  geom_point(data = test_Model_2, aes(y = (1-P_E_ME), size= N_ME, group =1), fill = "red", shape = 24)+
...
Pl_mod3_with_initial_data <- Pl_mod3_with_initial_data + geom_point(data = test_Model_3, aes(x = freq_MT, y = Accur, size = N),  shape = 24, fill = "red")
...
<!-- Дальнейший анализ должен показать, как ведут себя теоретические калькуляторы, подобранные в соответствии со стратегией, описанной выше. Но есть мнение (**которого я не придерживаюсь!**), что удобнее было бы приводить данные по генерализованной модели. Под "генерализованной" моделью  понимается модель, в которой фактор "Subset" имеет меньшее число градаций (или вовсе исключен из модели).  -->
...
1. W + BL + BH (все регионы входят в модель)
2. (WBL) + BH (объединены W BL)
3. (WBH) + BL (объединены W BH)
4. W + (BLBH) (объединены  BL BH)
5. (WBLBH)

Поблема заклчается в том, какую из моделей гонять в этом анализе. Поскольку наиболее адекватно, на мой взгляд, отражает работоспособность "morphotype-test" Model 4, то на ней и будет основан дальнейший анализ.
...
Приведенные данные показывают, что объединене W и BL не приводт к ухудшению регрессионной модели.  

## Theoretical models

Теоретическая модель 1, отражает аналитчески выводимую связь между Ptros и P_T, при заданных значениях P_Tm_T и P_Tm_E.

Теоретическая модель 2, отражает аналитически выводимую связь между вероятностью правильного определния генотипа по морфотипу и Ptros, при заданных P_Tm_T и P_Tm_E.


Далее, для выработки стартегии построения теоретических моделей мы будем работать с объединенными данными моделлингового датасета по Белому морю (W) и опресненного Баренува моря (BL). Данные по моделлинговому датасету полносоленого Баренцева моря (BH), где эмпирическая связь между морфотипами и генотипами слабее, в данном анализе не используются.



## Regression models for joined data

```{r}
## Визуализация новой модели с объединенными датасетами

mixed_data <- myt2[myt2$Subset %in% c("W", "BL"), ]

Model_6 <- glm(Sp2 ~  Prop_T , data = mixed_data, family = binomial(link = "logit"))

# overdisp_fun(Model6)

Model_6_final <- Model_6 

Model_6_final_summary <- tidy(Model_6_final)

Model_6_R2 <- r.squaredGLMM(Model_6_final)[1,1]


new_data6 <- mixed_data %>% group_by(pop) %>% summarise(Prop_T = mean(Prop_T) ) %>%  do(data.frame(Prop_T = seq(min(.$Prop_T), max(.$Prop_T), length.out = 100)))



predicted6 <- predict(Model_6_final, newdata = new_data6,  type="response", se.fit = T)

new_data6$fit <- predicted6$fit

new_data6$SE <- predicted6$se.fit 




Pl_mod6 <- ggplot(new_data6, aes(x = Prop_T, y = fit)) + geom_line(linetype = 2, color = "red", size = 1) +  geom_ribbon(aes(ymin = fit - 1.96*SE, ymax = fit + 1.96*SE), alpha = 0.1) + xlim(0, 1) + ylim(0, 1) +  geom_rug(data = candidat_data_2, inherit.aes = FALSE,  aes(x = Prop_T), size = 0.1) + geom_abline()
...
Model_7_full <- glmer(congr ~ morph * freq_MT + (1 | pop), data = mixed_data, family = binomial(link = "logit"), control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
Model_7_final_summary <- Model_7_final_summary[,!(names(Model_7_final_summary) %in% c("group"))]

Model_7_final_R2_m <- r.squaredGLMM(Model_7_final)[1,1]

Model_7_final_R2_c <- r.squaredGLMM(Model_7_final)[1, 2]




new_data7 <-mixed_data %>% group_by(morph) %>% do(data.frame(freq_MT = seq(min(.$freq_MT), max(.$freq_MT), length.out = 100)))

# Предсказанные значеня в шкале вероятностей
new_data7$fit <- predict(Model_7_final, newdata = new_data7, type = "response", re.form = NA)

# Предсказанные значеня в шкале логитов
new_data7$fit_eta <- predict(Model_7_final, newdata = new_data7, re.form = NA)

# Вычисление доверительного инеравала

X <- model.matrix(  ~ morph * freq_MT, data = new_data7) #Модельная матрица для визуализации


# Ошибки в шкале логитов
new_data7$se_eta <- sqrt(diag(X %*% vcov(Model_7_final) %*% t(X)))

new_data7$lwr <- logit_back(new_data7$fit_eta - 1.96 * new_data7$se_eta)

new_data7$upr <- logit_back(new_data7$fit_eta + 1.96 * new_data7$se_eta)



Pl_mod7 <- ggplot(new_data7, aes(x = freq_MT)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr, group = morph), alpha = 0.1)  +
  geom_line(aes(y = fit, color = morph), size=1, linetype = 2) +
...
## Regression models for joined White Sea and freshned Barents Sea



```{r}
model_joined_print <- rbind(empty_row, Model_6_final_summary, empty_row, Model_7_final_summary)


model_joined_print$estimate <- round(model_joined_print$estimate, 1)

model_joined_print$std.error <- round(model_joined_print$std.error, 2)

model_joined_print$statistic <- round(model_joined_print$statistic, 2)

model_joined_print$p.value <- round(model_joined_print$p.value, 3)

model_joined_print$p.value_print <- ifelse(model_joined_print$p.value < 0.001, "< 0.001", model_joined_print$p.value)

model_joined_print$term_print <- gsub("freq_MT", "Ptros", as.character(model_joined_print$term))
model_joined_print$term_print <- gsub("SubsetBL", "Subset(BL)", as.character(model_joined_print$term_print))
model_joined_print$term_print <- gsub("SubsetBH", "Subset(BH)", as.character(model_joined_print$term_print))
model_joined_print$term_print <- gsub("morphT_m", "Morph(T)", as.character(model_joined_print$term_print))
model_joined_print$term_print <- gsub("sd_(Intercept).pop", "SD(Intercept)", as.character(model_joined_print$term_print))
model_joined_print$term_print <- gsub("SpM.trossulus", "Species(*M.trossulus*)", as.character(model_joined_print$term_print))
model_joined_print$term_print <- gsub("Prop_T", "PT", as.character(model_joined_print$term_print))



model_joined_print_print <- model_joined_print[, c("term_print", "estimate", "std.error", "statistic", "p.value_print" )] 

model_joined_print_print$term_print [is.na(model_joined_print_print$term_print)] <- c("**Model 6**", "**Model 7**")


# Вставляем данные pseudo Rsq
model_joined_print_print$estimate [is.na(model_joined_print_print$estimate)] <- c(
  paste("$pseudo R^2$ = ", round(Model_6_R2, 2)),
  paste("$pseudo R^2_{m}$ = ", round(Model_7_final_R2_m, 2))
)


model_joined_print_print$std.error [is.na(model_joined_print_print$std.error)] <- c(
  NA,
  paste("$pseudo R^2_{c}$ = ", round(Model_7_final_R2_c, 2)),
  NA
  )
...
# options(knitr.kable.NA = ' ')
kable(model_joined_print_print, col.names = c("Terms", "Estimate", "SE", "z-statistic", "p-value"))
...
Pl_teor_empir_6 <- ggplot(perms2(df = mixed_data), aes(x = Delta, y = Goodness)) + geom_point(size = 0.1) + geom_smooth(se = F) + labs(y = "Goodness \n")

Pl_teor_empir_6 <- Pl_teor_empir_6 + ggtitle("Regression Model 6 \nvs Theoretical model 1")

Pl_teor_empir_7 <- ggplot(perms4(df = mixed_data), aes(x = Delta, y = Goodness)) + geom_point(size = 0.1) + geom_smooth(se = F) + labs(y = "Goodness \n")

Pl_teor_empir_7 <- Pl_teor_empir_7 + ggtitle("Regression Model 7 \nvs Theoretical model 2")
...
Figure +. Correspondence between regression and theoretical models. Each point corresponds to one of the possible pairs of populations from modelling data set (White Sea joined with low salinity Barens Sea).  OX axis represents the differencу in genetic structre for each pair of populations. OY axis represents correspondence between prediction of regression model and theoretical model. Lines represent loess-smoother. (A ) Model 6 describing  the dependence of proportion of M.trossulus (Ptros) on proportion of T-morphotype (P_T) ; (B) Model 4  describing  the dependence of probability of correct species identification (Pcorrect) on  proportion of M.trossulus (Ptros)  and morphotype (Morph). 
...
pops_max_mix <- max_mix(df = mixed_data, Subset = c("W", "BL"))
...
Pl_calc1_mod6 <- Pl_mod6 + geom_line(data = calc1_W, aes(x = P_T, y = Ptros), color = "blue", size = 1) + geom_line(data = calc1_B, aes(x = P_T, y = Ptros), color = "gray")  + geom_point(data = myt5, aes(x = Prop_T, y = freq_MT), size = 4, color = "red")  + labs(x = "Proportion of T-morphotype", y = "Proportion of M.trossulus \n")
...
<!-- test_pop <- myt5 %>% group_by(pop) %>% summarize(Prop_T = mean(Prop_T), freq_MT = mean(freq_MT)) -->

<!-- test_pop$P_T_Calc1_predict <- c( -->
<!--   (coef_calc1_W[1] + coef_calc1_W[2] *  test_pop %>% pull(Prop_T)) -->
<!--   ) -->
...
<!-- dd <- merge(myt5, test_pop[ , c("pop", "P_T_Calc1_predict")], by = c("pop")) -->

<!-- myt5_with_predictions <- merge(dd,coef_calc2) -->

<!-- myt5_with_predictions$Calc2_predictions <- with(myt5_with_predictions, ifelse(morph == "T_m", -->
...
<!-- test_calc2_summary <- myt5_with_predictions %>% group_by(pop, morph) %>% summarise(Pcorrect = mean(congr), Calc2_pred = mean(Calc2_predictions)) -->

<!-- ggplot(test_calc2_summary, aes(x = Calc2_pred, y = Pcorrect, color = morph)) +  facet_grid(~morph ) + geom_abline() + xlim(0,1)+ ylim(0,1) + geom_text(aes(label = pop)) + labs(x = "Probabilit of correct identification predicted by theoretical Model 2", y = "Observed probability of correct identification") -->
