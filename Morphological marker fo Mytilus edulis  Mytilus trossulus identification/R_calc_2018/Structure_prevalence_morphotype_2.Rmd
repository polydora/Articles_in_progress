...
Pl_ed <-
  ...
  geom_point(size = 4) + 
  geom_abline() +
  labs(x = "P(T|edu)", y = "PT in putative purebred M.edulis (q<0.2)", color = "Region")

Pl_tr <-
  ggplot(PT_pure_trssulus_like, aes(x= Prop_T_Sp_like, y = Prop_T_Sp_pure, color = Subset)) + 
  geom_point(size = 4) + 
  geom_abline() +
  labs(x = "P(T|tros)", y = "PT in putative purebred M.trossulus (q>0.8)", color = "Region")
...
```{r}
myt2_all %>% group_by(sea, Sp) %>% summarise(Prop_T = mean(ind)*100)


PT_sp_like <- myt2_all %>% group_by(Subset, Sp) %>% summarise(Prop_T_Sp_like = mean(ind == 1))
...
PT_sp_pure <- myt2_all %>% group_by(Subset,  Sp_pure) %>% summarise(Prop_T_Sp_pure = mean(ind == 1)) %>%  filter(Sp_pure != "hybrid")

names(PT_sp_pure)[2] <- "Sp"
...
data.frame(Set = PT_pure_edulis_like$Subset,  PT_edu = round(PT_pure_edulis_like$Prop_T_Sp_like, 2), PT_pure_edu = round(PT_pure_edulis_like$Prop_T_Sp_pure, 2), PT_tros = round(PT_pure_trssulus_like$Prop_T_Sp_like, 2), PT_pure_tros = round(PT_pure_trssulus_like$Prop_T_Sp_pure, 2))
```
