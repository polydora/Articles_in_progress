...
Pl_ed <-
  ggplot(PT_pure_edulis_like, aes(x= Prop_T_Sp_like, y = Prop_T_Sp_pure)) + 
  geom_point() + 
  geom_abline() +
  labs(x = "P(T|edu)", y = "PT in putative M.edulis (q<0.2)")

Pl_tr <-
  ggplot(PT_pure_trssulus_like, aes(x= Prop_T_Sp_like, y = Prop_T_Sp_pure)) + 
  geom_point() + 
  geom_abline() +
  labs(x = "P(T|tros)", y = "PT in putative M.trossulus (q>0.8)")


plot_grid(Pl_ed, Pl_tr, labels = c("A", "B"), ncol = 2)
```
