...
Pl_mod7_with_initial_data <- Pl_mod7 + geom_point(data = init_data_Model_7, aes( y = freq_MT), shape = 22, size = 2 ) + scale_fill_continuous(low = "white", high = "black") + labs(y = "Ptros", x = "PT")
...
WSBL_formula <- data.frame(Subset = "WSBL", Prop_T = seq(0, 1, 0.01)) %>% mutate(fit = exp(-2.4+5.4*Prop_T)/(1+exp(-2.4+5.4*Prop_T)))

BH_formula <- data.frame(Subset = "BH", Prop_T = seq(0, 1, 0.01)) %>% mutate(fit = exp(-3.9+5.0*Prop_T)/(1+exp(-3.9+5.0*Prop_T)))

GOM_formula <- data.frame(Subset = "GOM", Prop_T = seq(0, 1, 0.01)) %>% mutate(fit = exp(-2.3+6.2*Prop_T)/(1+exp(-2.3+6.2*Prop_T)))

BALT_formula <- data.frame(Subset = "BALT", Prop_T = seq(0, 1, 0.01)) %>% mutate(fit = exp(-0.6+11.6*Prop_T)/(1+exp(-0.6+11.6*Prop_T)))

NORW_formula <- data.frame(Subset = "NORW", Prop_T = seq(0, 1, 0.01)) %>% mutate(fit = exp(-0.5+3.7*Prop_T)/(1+exp(-0.5+3.7*Prop_T)))

formula1 <- rbind(WSBL_formula, BH_formula, GOM_formula, BALT_formula, NORW_formula)

Pl_mod7 + geom_line(data = formula1, color = "blue")
