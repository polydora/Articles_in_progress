...
## Предложения по структуре работы

Я предлагаю следюющую логику изложения материала.

Часть 1.    
Мы анализируем связь между частотой T-морфотипа и вероятностью встретить в популяции M.trossulus (среднее значение structure в популяции). Это дает нам два результата: 
1) Роль солености и моря в фомировании этой связи (это важно показать сразу, так как далее sal_place и sea будут везде играть роль важных предикторов).
2) Мы получм уравнения для предсказания вероятности встретить M.trossulus в популяци если мы знаем частоту T-морфотипа.




Часть 2.   
Поскольку связь описанная выше хорошо читается, то наша задача проанализировать насколько морфологический признак хорошо работает, ка кдиагностический инструмент. Традиционным методом для подобоо анализа является анализ ROC-кривых. Из ROC-анализа мы выносим знание, что диагностическая ценность признака высока в Белом море повсеестно, а в Баренцевом море только в местах с опреснением. В Номально соленых местах диагностическая сида морфологического признака резко снижается. 


Часть 3.    
Поскольку признак работает лишь с некотрой вероятнсотью (это следует из ROC-анализа) в силу вступает баесовские закономерности. То есть мы должны знать, как ведет себя вероятность правльного определения вида по морфотипу в зависимости от того какова веротяность встретить M.trossulus в популяции. Здесь мы строим баесовские кривые и подбираем модели, описывающеие вероятность правиьного определения в зависимости от следующих предикторов:     
1) Вероятности встретить M.trossulus, что требует баесовский подход,    
2) sea и sal_place, поскольку мы уже заем, что они играют важную роль.  



Часть 4.    
Мы проверяем на тестовых выборках, работает ли модель, которую мы предложили для предсказания вероятностей правильного определения видов. 

At! Для полноты картины ОЧЕНЬ было бы правильно выделить хотя бы какое-то количество популяций из Белого моря для тестового датасета. А то криво получается -- для Баренцева моря мы проверяем работоспособность, а для Белого нет.    


Часть 5.    
Мы показываем, на разных географических районах генетическую структуру (вероятность встретить M.trossulus) и частоту морфотипов. Это дает основания для баесовской оценки диагностической роли морфологического признака в разных регионах.
...
## Часть 1. Зависимость веротяности встречи M.trossulus от частоты T-морфотипа в популяциях
...
new_data2 <- expand.grid(sea = c("barents", "white"), sal_place = c("fresh", "normal"), Prop_T = seq(0, 1, 0.01))
new_data2$Prop_MT <-  predict(Model_T_MT2, newdata = new_data2,  type="response")
...
ggplot(prop_T_MT, aes(x = Prop_T, y = prop_MT_adj, color = sal_place)) + geom_point(aes(color = sal_place))+ geom_line(data = new_data, aes(x = Prop_T, y = fit, color = sea, linetype = sal_place), size = 1) + theme_bw() + labs(x = "Proportion of T-morphotype in population", y = "Proportion of M.trossulus", color = "Freshness") + geom_abline()
...
#Данные без минорных генотипов

count_MT_ME <- myt2 %>% group_by(pop, Sp) %>% summarize(N = n())

count_MT_ME <- dcast(pop ~ Sp, data=count_MT_ME)

count_MT_ME[is.na(count_MT_ME)] <- 0

count_MT_ME$pop

include <- count_MT_ME$pop[count_MT_ME$M.edulis > 4 & count_MT_ME$M.trossulus > 4]

myt2 <- myt2[myt2$pop %in% include, ]
...
Mod_fT_congr_fin <- Mod_fT_congr4
...
X <- model.matrix(  ~ ~ morph + freq_MT + sea + sal_place + morph:freq_MT + 
    morph:sal_place + freq_MT:sea + sea:sal_place, data = newdata) #Модельная матрица для визуализации
...
ggplot(test_fr_MT, aes(x = Prop_MT_obs, y = Prop_MT_predicted, color = sal_place)) + geom_point(size = 4) + geom_abline() + theme_bw() + labs(x = "Observed proportion of M.trossulus", y = "Predicted poportion of M.trossulus")
