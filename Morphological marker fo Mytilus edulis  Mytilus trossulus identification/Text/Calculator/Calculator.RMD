...
Pl_ptros <- ggplot(df_bayes, aes(x = P_T, y = Ptros_predicted)) + 
  ...
  ggtitle("Taxonomic structure of mixed population")
...
Since $P_{tros|T}$ and $P_{edu|E}$ are dependent on $Ptros$ the parameter $Ptros$ could be evaluated using data on $PT$. 

**Recommendation**:  One should choose the desirable minimal level of probability of correct identification ($P_D$) for each species. It give the possibility to assess the threshold value of $PT$ which allows to separate minor species for which probability of correct identification is lesser than chosen $P_D$ (morphotype-test is not recommended for use in this case). These threshold values could be assessed by formulas as follows.
