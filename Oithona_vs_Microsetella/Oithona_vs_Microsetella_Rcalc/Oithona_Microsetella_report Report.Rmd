...
anova(oit_cca_reduced, permutations = 99999)

anova(oit_cca_reduced, by = "axis", permutations = 9999)

anova(oit_cca_reduced, by = "margin", permutations = 9999)
...
Mod <- lm( log(oit_N2$Male + oit_N2$Female +1)  ~  . , data = predictors)
...
Mod_reduced <-lm(formula = log(oit_N2$Male + oit_N2$Female + 1) ~ Year + Nauplii + 
    Cop. + Adult_1 + t_peak + Sp_start + Su_end + Su_start_1 + 
    t_Spring + NAO, data = predictors)
...
qplot(x = oit_N2$Year, y = residuals(Mod_reduced)) + geom_smooth(method = "lm")

acf(residuals(Mod_reduced))
