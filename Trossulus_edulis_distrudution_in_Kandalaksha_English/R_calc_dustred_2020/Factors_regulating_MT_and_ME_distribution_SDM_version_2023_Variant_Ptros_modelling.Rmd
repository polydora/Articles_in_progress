...
Mod_gam <- gam(Ptros ~ s(Salinity, bs = "cs") + s(Min_dist_river, bs = "cs") + s(Fetch, bs = "cs") + s(Min_dist_port, bs = "cs") + Position + River_Size + Port_Status + s(Site, k = Total_site, bs = "re"), method = "REML", family = betar(link = "logit"), data = myt_full )
...
myt_full <- 
  ...
  mutate(Pi_Salinity = logit_back(`s(Salinity)` + coef(Mod_gam)[1]),
         ...
         Pi_Average_Fetch = logit_back(`s(Fetch)` + coef(Mod_gam)[1]),
...
Pl_fetch_smoother <-
  ...
  ggplot() +
  geom_point(aes(x = Fetch, y = `s(Fetch)`, color = Position),
             data = myt_full, cex = 1, colour = "steelblue3") +
  geom_rug(aes(x = Fetch),
           data = myt_full,
           sides = "b", length = grid::unit(0.02, "npc")) +
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, x = Fetch),
              alpha = 0.2) +
    geom_line(aes(x = Fetch, y = est), lwd = 1) +
