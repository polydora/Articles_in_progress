...
myt_site <- myt_full %>% 
  group_by(Shore, Site) %>% 
  select(Lat, Lon, N_T, N_E, Salinity, Min_dist_river, River, River_Size, Min_dist_river_Large, Min_dist_port, Port, Port_Status, Average_Fetch, Dist_cut, Log_Min_dist_port, Log_Min_dist_river, Log_Average_Fetch) %>% 
...
Species distribution models (SDM) being a numerical tools describing the relationship between species occurrence and environmental parameters  allow to predict distribution patterns of ecologically and economically important species both in space and time (Elith, Leathwick, 2009). The SDM framework is very popular in terrestrial ecology (+++) but the amount of papers devoted to SDMs for marine species has been growing significantly in recent years as well (Robinson et al 2017).SDMs are mostly applied for «good», morphologically distinct, species that can be involved in routine studies which in the case of marine ecology  require numerous samples (e.g. Reiss et al., 2011; Lindegren et al., 2022). However, the increasing evidence for cryptic species among marine animals (Bastrop et al 1998; Fiser et al 2010) makes it urgent to find possibility to build SDMs for sympatric or parapatric cryptic species (e.g.Dennis, Hellberg, 2010), and even to infraspecific taxa such as subspecies or phylogenetic lineages within polytypic species (e.g. Lowen et al 2019). 
...
In the case of coexisting  sympatric taxa (including cryptic ones) SDM approach  could be extended using the Joint Species Distribution Modelling (JSDM) approach (Ovaskainen, Abreg, 2020). Whereas the basic SDM assume to use the presence/absence (or abundance) of a species as a response variable, which is under control of numerous environmental predictors, in JSDM framework some correlative assessment of interspecific interactions are incorporated into the model as well (Ovaskainen, Abreg, 2020). While the SDMs are considered as a way of discribe of ecological niches of single species (Elith, Leathwick, 2009), the implementation of the JSDM approach can directly consider the manifestation of ecological niche partitioning between species in a community (Ovaskainen, Abreg, 2020 +++).



The presence of several mussel species, the strict identification of which requires the use of expensive genetic tests, significantly complicates ecological studies.    
...
Mod_gam <- gam(Ptros ~ s(Salinity, bs = "cr") + s(Log_Min_dist_river, bs = "cr") + s(Log_Average_Fetch, bs = "cr") + s(Log_Min_dist_port, bs = "cr") + Position + River_Size + Port_Status + s(Site, k = Total_site, bs = "re"), method = "REML", family = betar(link = "logit"), data = myt_full )

Mod_gam_reduced <- gam(Ptros ~ s(Log_Min_dist_river, bs = "cr") + s(Log_Average_Fetch, bs = "cr") + s(Log_Min_dist_port, bs = "cr") + Position + River_Size + Port_Status + s(Site, k = Total_site, bs = "re"), method = "REML", family = betar(link = "logit"), data = myt_full )



Mod_gam_ML <- gam(Ptros ~ s(Salinity, bs = "cr") + s(Log_Min_dist_river, bs = "cr") + s(Log_Average_Fetch, bs = "cr") + s(Log_Min_dist_port, bs = "cr") + Position + River_Size + Port_Status + s(Site, k = Total_site, bs = "re"), method = "ML", family = betar(link = "logit"), data = myt_full )


Mod_gam_reduced_ML <- gam(Ptros ~ s(Log_Min_dist_river, bs = "cr") + s(Log_Average_Fetch, bs = "cr") + s(Log_Min_dist_port, bs = "cr") + Position + River_Size + Port_Status + s(Site, k = Total_site, bs = "re"), method = "ML", family = betar(link = "logit"), data = myt_full )
...
myt_full <- 
  ...
  mutate(Pi_Salinity = logit_back(`s(Salinity)` + coef(Mod_gam)[1]),
         ...
         Pi_Average_Fetch = logit_back(`s(Log_Average_Fetch)` + coef(Mod_gam)[1]),
...
Pl_riv_dist_smoother <- 
  ...
  geom_point(aes(x = Log_Min_dist_river, y = `s(Log_Min_dist_river)`, color = Position),
             data = myt_full, cex = 1, colour = "steelblue3") +
  geom_rug(aes(x = log(Min_dist_river)),
           data = myt_full,
           sides = "b", length = grid::unit(0.02, "npc")) +
...
Pl_fetch_smoother <-
  sm %>%
  filter(smooth == "s(Log_Average_Fetch)") %>%
  ggplot() +
  geom_point(aes(x = Log_Average_Fetch, y = `s(Log_Average_Fetch)`, color = Position),
             data = myt_full, cex = 1, colour = "steelblue3") +
  geom_rug(aes(x = Log_Average_Fetch),
           data = myt_full,
           sides = "b", length = grid::unit(0.02, "npc")) +
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, x = Log_Average_Fetch),
              alpha = 0.2) +
    geom_line(aes(x = Log_Average_Fetch, y = est), lwd = 1) +
  labs(y = "Partial effect", x = "Log Average Fetch") +
  geom_hline(yintercept = 0, linetype = 2) +
  theme_bw()
...
Pl_port_dist_smoother_Pi <-
  ...
  filter(smooth == "s(Log_Min_dist_port)") %>%
  ggplot() +
  geom_point(aes(x = Log_Min_dist_port, y = Pi_Min_dist_port), data = myt_full, cex = 1, colour = "steelblue3") +
  geom_rug(aes(x = Log_Min_dist_port),data = myt_full, sides = "b", length = grid::unit(0.02, "npc")) +
  geom_ribbon(aes(ymin = CI_Pi_low, ymax = CI_Pi_up, x = Log_Min_dist_port),alpha = 0.2) +
  geom_line(aes(x = Log_Min_dist_port, y = Pi), lwd = 1) +
  labs(y = "Partial effect", x = "Log Distance to the nearest port (km)") +
...
Pl_fetch_smoother_Pi <-
  ...
  filter(smooth == "s(Log_Average_Fetch)") %>%
  ggplot() +
  geom_point(aes(x = Log_Average_Fetch, y = Pi_Average_Fetch), data = myt_full, cex = 1, colour = "steelblue3") +
  geom_rug(aes(x = Log_Average_Fetch),data = myt_full, sides = "b", length = grid::unit(0.02, "npc")) +
  geom_ribbon(aes(ymin = CI_Pi_low, ymax = CI_Pi_up, x = Log_Average_Fetch),alpha = 0.2) +
  geom_line(aes(x = Log_Average_Fetch, y = Pi), lwd = 1) +
...
Mod_Dif <- gam(Dif_Ptros ~ s(Total_Ptros, bs = "cr") + s(Log_D_total, bs = "cr"), method = "REML", data = all_sites)
...
Mod_gam_prediction_tuv <- predict(Mod_gam, newdata = tuv2, type = "response", exclude = "s(Site)")

roc_Mod_gam_tuv <- roc(response = tuv2$Mt_dominated, predictor = Mod_gam_prediction_tuv)
...
a <- sum(myt_site_substr$Prop_T > boundary & Mod_gam_prediction_train > boundary)

b <- sum(myt_site_substr$Prop_T < boundary & Mod_gam_prediction_train > boundary)

c <- sum(myt_site_substr$Prop_T > boundary & Mod_gam_prediction_train < boundary)

d <- sum(myt_site_substr$Prop_T < boundary & Mod_gam_prediction_train < boundary)
...
a <- sum(myt_test_site$Prop_T > boundary & Mod_gam_prediction_test > boundary)

b <- sum(myt_test_site$Prop_T < boundary & Mod_gam_prediction_test > boundary)

c <- sum(myt_test_site$Prop_T > boundary & Mod_gam_prediction_test < boundary)

d <- sum(myt_test_site$Prop_T < boundary & Mod_gam_prediction_test < boundary)
...
# For Tyuva data set
a <- sum(tuv$PT > boundary & Mod_gam_prediction_tuv > boundary)

b <- sum(tuv$PT < boundary & Mod_gam_prediction_tuv > boundary)

c <- sum(tuv$PT > boundary & Mod_gam_prediction_tuv < boundary)

d <- sum(tuv$PT < boundary & Mod_gam_prediction_tuv < boundary)
...
Pl_Tuva_testing <-
  qplot(x = tuv2$Ptros_predicted,  y = Mod_gam_prediction_tuv) +
  labs(x = "Obseved Ptros", y = "Predicted Ptros") +
  geom_point(shape = 21, aes(fill = tuv2$Position)) +
  scale_fill_manual(values = c("white", "black")) +
  guides(fill= "none") +
  geom_abline(linetype = 2) +
  geom_vline(xintercept = boundary) +
