---
title: "Mixed populations of Mytilus trossulus and M.edulis in the White Sea: towards to a species distribution model"
...
df2 <-
myt_site_substr %>% 
  select(Site, Position, N_T) %>%
  dcast(., formula = Site ~ Position, value.var = c("N_T") )


names(df2) <- c("Site", "N_T_Bottom", "N_T_Algae" )


df3 <-
myt_site_substr %>% 
  select(Site, Position, N_E) %>%
  dcast(., formula = Site ~ Position, value.var = c("N_E") )

names(df3) <- c("Site", "N_E_Bottom", "N_E_Algae" )

all_sites <- merge(df1, df2) %>% merge(., df3) %>% mutate(Total_Prop_T = (N_T_Bottom + N_T_Algae)/(N_E_Bottom + N_E_Algae +  N_T_Bottom + N_T_Algae), N_total = log(N_E_Bottom +  N_T_Bottom), Dif = log( (Prop_T_Algae + 1) / (Prop_T_Bottom + 1) ) ) 

Mod_Dif <- gam(Dif ~ s(Total_Prop_T) + s(N_total), data = all_sites)

summary(Mod_Dif)
draw(Mod_Dif, residuals = T) + geom_hline(yintercept = 0)
