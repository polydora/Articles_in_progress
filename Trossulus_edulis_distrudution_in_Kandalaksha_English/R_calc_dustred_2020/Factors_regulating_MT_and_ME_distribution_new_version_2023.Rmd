...
To check for the predictors' collinearaity we calculated the variance inflation factor (VIF, Fox & Monette, 1992) for each of them. Additionally we calculated Pearson correlation between continuous predictors. 

To verify that the *Model 1* meets the assumptions of sampling independence, we examined the presence of  residuals' spatial autocorrelation by means of semivariogram construction (++). Only poor evidences of spatial autocorrelation were revealed. 



At the second step, we constructed the frequency distribution of *PT* values. We revealed a condidtional boundary between peaks in distribution corresponding to *ME*-dominated populations (low *PT* values) and *MT*-dominated ones (high *PT*). Thus we divided our samples into two groups. We constructed a variable marking all populations from the "*ME*-dominated" group as "0", and as "1" populations from the "*MT*-dominated" one. We used this variable as dependent one in the logistic mixed effect model (thereafter *Model 2*), describing the probability of a population to be *Mt*-dominated as a function of the same predictor set as in the *Model 1*. "Site" was considered as random factor in the *Model 2*. The model was fitted with "glmer()" finction from "lme4" package (+++). 

The fraction of total variability explained by the fixed part of the *Model 2* was estimated using marginal coefficient of determination  *R^2^* (Nakagawa & Schielzeth, 2013). Marginal *R^2^* can be decomposed into semi-partial coefficients of determination, *semi-partial R^2^*, that describe the contribution of each individual predictor to the overall coefficient of determination using the “partR2” package  (Stoffel et al., 2021). We calculated the "inclusive R^2^" to estimate the total variance
explained by a predictor (Stoffel et al., 2021).
...
ggplot(., aes(x = Position, y = Pi, fill = Port_Status)) + 
  ...
  ylim(0, 0.7)+
  theme(legend.position = c(0.19, 0.7)) +
...
To assess the dependency of *PT* on all predictors considered we constructed the *Model* (Table ++, R^2^ = `r round(sum_Model_1$r.sq, 2)`) which explained  `r round(sum_Model_1$ dev.expl*100, 1)`% of total deviance. The *Model* revealed statistically significant dependency of *PT* on all predictors except distance to the nearest river (*MinDistRiver*).
...
According to the *Model* fitted (Table ++, Fig. ++) the value of *PT* is dramatically higher on the Algae substrate than on the Bottom one. This value is higher in those sites which is close to active ports than to abandoned ones. Finally, the *PT* value is slightly higher at sites situated close to large rivers than to small ones. The *PT* has a tendency to decrease with increasing of distance to the nearest port (Fig. ++, D) starting to be in average lower than median partial effect when distance to the nearest port is higher than 20 km. The dependency of *PT* on average fetch (Fig. ++, E) is also negative: *PT* become lover than median partial effect when value of fetch started to be higher of 5 km.   

The curvilinear dependence of *PT* on salinity (Fig. ++, F) can be described as follows. The predicted value of *PT* is close to the median partial effect at intermediate salinity range. However, at extremely low (less than 10 ppt) and high (more than 25 ppt) salinity, *PT* becomes higher than the median predicted value.    


## Assessment of predictor's importance

The values of *PT* ranged from `r round(min(myt_site$Prop_T), 0)` to `r round(max(myt_site$Prop_T), 2)` at different sites. The frequency distribution of *PT* values displayed a clear bimodal structure both in the case of "Bottom" and "Algae" samples (Fig. ++). Thus two types of populations can be recognized in the studied area: populations dominated by *ME* (conditional boundary *PT* < 0.5) and samples with MT prevalence (*PT* > 0.5). In collections from the "Bottom" substrate, samples belonging to the first group were more frequent (the left peak in the distribution predominated). In collections from the "Algae" substrate, both groups of specimens were represented in more or less equal proportions.
...
myt_full$Fi_T <- 2*asin(sqrt(myt_full$Prop_T))*180/pi

binwidth <- 0.05

Pl_PT_distribution_substrates <-
ggplot(myt_full, aes (x = Prop_T)) + 
...
x <- 2*asin(sqrt(myt_full$Prop_T)) *180/pi

model <- normalmixEM(x=x, verb = F)

# model <- gammamixEM(x=x, verb = F)
...
myt_full <- 
  myt_full %>% 
  mutate(T_dominated = ifelse(Prop_T > 0.5, 1, 0))

myt_site_substr <- 
myt_full %>% 
  group_by(Site, Position) %>% 
  summarise(Prop_T = sum(N_T)/(sum(N_T) + sum(N_E)),
...
Model_2 <- glmer(T_dominated ~ Position + Salinity + Min_dist_river + River_Size + Average_Fetch + Min_dist_port + Port_Status + (1|Site) ,  data = myt_full, family = binomial(link = "logit"), control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)) )

summary(Model_2)


qplot(x= fitted(Model_2), y = residuals(Model_2, type = "pearson")) + geom_smooth(method = "lm") + geom_hline(yintercept = 0)

library(partR2)

# res_part_R2 <- partR2(Model_2, nboot = 100)
...
Parameters of the *Model 2* are presented in STable ++. The marginal coefficient of determination of the *Model 2* (R^2^ = `r round(res_part_R2$R2$estimate, 2)`) was portioned as follow (Fig. +). The highest 
...
Using the "morphotype test" (Khaitov et al. 2021) as a tool for species identification, we were able to construct a *Model* describing the relationship between *PT* frequency and all the factors that are generally considered to be the main regulators of the *MT:ME* ratio in mixed mussel settlements.  Our analysis showed that the predictors included in the model, although weakly correlated with each other, do not exhibit high collinearity. This means that we can consider all factors involved as independent entities that do not mask each other in their influence.   

We found that all factors, which were considered as crucial for *MT:ME* proportion regulation in previous works, were significant for *PT* value (and consequently proportion of *MT* in mixed populations) in a small contact zone of the Kandalaksha Bay. This means that the patterns revealed on more extensive contact zones with longer ecological gradients retain their influence in essentially smaller spatial scales. 
...
It is believed that *MT* was introduced into the Barents and White Seas due to ship traffic during World War II (Vainola & Strelkov, 2011). This hypothesis was based primarily on the fact that high concentrations of *MT* were found in the vicinity of currently active or abandoned ports in the Barents and White Seas (Vainola & Strelkov, 2011; Katolikova et al. 2016). Our study confirmed this hypothesis. The proportion of T-morphotype (and hence *MT*) in the studied area decreases with the distance from the port.
...
An intriguing result of our study was the absence of clear linear signal of the influence of salinity on the *MT:ME* ratio. GAM fitted reveal a statistically significant curvelinear relationship between *PT* and salinity. From one hand, our *Model* revealed expected dependency on salinity: *PT*, in fact, was higher in the lower salinity conditions. However, on the other hand, it was unexpectedly high when salinity was unusually high for the region. Such highly saline regions were uevnly distributed around area. 

Про то, что в большинстве участков, когда соленость нормальная,  доминирует E
Гипотеза маргинальных местообитаний для T.
Сказать пр несовершенстов методов определиня солености непосредственно.
Про возможность отращивать T-морфотип при высокой солености в арктике.




We attempted to estimate the degree of desalination indirectly by introducing into the model quantities that may alternatively characterize salinity at the sampling site: the distance to the nearest river and the size of the nearest river. Only the second had significant influence: *PT* was higher if the nearest river was large. Thus using this type of salinity proxy 


Both of these quantities, like salinity, contributed negligibly to the total variance explained by the model. This result is in contradiction with the fact that many papers analyzing *MT* distribution in *MT-ME* contact zones indicate that *MT* forms populations in more desalinated habitats and *MT* gene frequencies decrease as salinity increases (Riginos and Cunningham, 2005; Katolikova et al., 2016; Stuckas et al., 2017; Kijewski et al 2019). 
