...
myt_full$Year <- as.numeric(myt_full$Year) 
...
+++++++++++++++++
**Dependency of mussels abundance on environmental parameters in modeling dataset (Model 2)**

For this part of the analysis, we used a matrix of abundance of T-mophotype and E-morphotype mussels. A mean abundance was calculated for each morphotype in each site, without considering the substrate on which samples were taken. The omission of substrate as a factor in this case was due to the fact that mussel abundance on different substrates was estimated by different techniques (see above). The latter is not crucial for calculation of such relative values as *Ptros* but make it impossible to isolate the effect of abundance estimation technique from the effect of substrate type. 

The resulting mean abundances of the two morphotypes in each site were log-transformed and used as the dependent variable in the **Model 2**. This model was constructed as a generalized additive model (GAM with Gaussian residuals’ distribution) and included the same set of predictors as **Model 1**, except the factor “Substrate” and the random factor “Site”. The factor “Morphotype” was included in the **Model 2**, as well as its interactions with other predictors. 

The model validity was assessed following the same approach as in the case of **Model 1**. No collinearity of predictors and spatial patterns in the residuals was detected.
...
myt_full <-
  myt_full %>% 
  mutate(N_total = N_T + N_E, N_tros = round(Ptros *N_total, 0), N_edu = N_total - N_tros) 
...
# Mod_gam <- gam(Ptros ~ s(Salinity, bs = "cs") + s(Log_Min_dist_river, bs = "cs") + s(Log_Fetch, bs = "cs") + s(Log_Min_dist_port, bs = "cs") + Position + River_Size + Port_Status, method = "REML", family = betar(link = "logit"), data = myt_site_substr )



# appraise(Mod_gam, use_worm = F, method = "simulate", type = "deviance")
# 
# draw(Mod_gam)

# library(DHARMa)
# simulationOutput <- simulateResiduals(Mod_gam, n = 250, refit = F )
# 
# plotQQunif(simulationOutput)
# 
# plotResiduals(simulationOutput,  quantreg = T, rank = T, smoothScatter = T)
# 
# testQuantiles(simulationOutput)
# 
# plotResiduals(simulationOutput, form = myt_full$Position)
# 
# testSpatialAutocorrelation(simulationOutput, x = myt_site_substr$Lon, y = myt_site_substr$Lat)
# 
# testQuantiles(simulationOutput)
# 
...
# 
# testCategorical(simulationOutput, catPred = myt_site_substr$Position)
# 
# E_dharm <- residuals(simulationOutput)
# 
# Pred <- predict(Mod_gam)
# 
# logit_back <- function(x) exp(x)/(1 + exp(x)) 
# 
# Pred_pi <- logit_back(Pred)  
# 
# E_pears <- residuals(Mod_gam, type = "pearson")
# 
# 
# qplot(as.numeric(myt_full$Year), E_pears) + geom_smooth() +geom_hline(yintercept = 0)
# summary(gam(E_pears ~ s(myt_full$Year, k = 7)))
# # Нет временного паттерна в остатках
# 
# 
# qplot(as.numeric(myt_full$Year), E_dharm) + geom_smooth() +geom_hline(yintercept = 0.5)
# 
# 
# 
# 
# qplot(Pred_pi, E_dharm) + geom_smooth()+ geom_hline(yintercept = 0.5)
# 
# 
# 
# qplot(myt_site_substr$Ptros, E_pears) + geom_smooth() 
# 
# qplot(myt_site_substr$Ptros, Pred_pi ) + geom_smooth() + geom_abline()
# 
# hist(abs(myt_site_substr$Ptros-Pred_pi))
# median(abs(myt_site_substr$Ptros-Pred_pi))
# 
# mod_foo <- lm(Pred_pi ~ myt_site_substr$Ptros)
# 
# 
# 
# qplot(myt_site_substr$Prop_T, Pred_pi) + geom_smooth() + geom_abline()
# 
# qplot(E_pears, E_dharm)
# 
# qplot(E_pears, E_dharm)
# 
...
myt_full <- 
  myt_full %>% 
  cbind(partial_residuals(Mod_gam))
...
# 
# 
# myt_site_substr <- cbind(myt_site_substr, partial_residuals(Mod_gam))
...
# data_slice(Mod_gam,  Salinity = evenly(Salinity, n = 100)) %>% 
#   fitted_values(Mod_gam, data = ., scale = "response", exclude = "s(Site)") %>% 
#   ggplot(aes(x = Salinity, y = .fitted)) +
#   geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci), alpha = 0.2) +
#    geom_line() + 
#   geom_point(data = myt_full, aes(x = Salinity, y = `s(Salinity)`, color = Position), cex = 1, colour = "steelblue3") 
...
Pl_riv_dist_smoother <- 
  ...
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, x = Log_Min_dist_river),
              alpha = 0.2) +
  geom_line(aes(x = Log_Min_dist_river, y = .estimate), lwd = 1) +
...
Pl_fetch_smoother <-
  ...
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, x = Log_Fetch),
              alpha = 0.2) +
  geom_line(aes(x = Log_Fetch, y = .estimate), lwd = 1) +
...
Mod_dummy <- lm(Prop_T ~ Salinity + Min_dist_river + Fetch + Min_dist_port + Position + River_Size + Port_Status,  data = myt_site_substr )
...
```{r Data on Abundance of T- and E-morphotype}
...
abundance <- 
  all_sites %>% 
  dplyr::select(Log_Bottom_D_T, Log_Bottom_D_E, Log_Algae_D_T, Log_Algae_D_E)
...
```{r Absolute abundance modelling}
# Model for absolute abundance
# 
# # Assessment of abyndance by PC1 and PC2
# 
all_sites <-
  merge(myt_site, all_sites)

all_sites$Log_Min_dist_river <- log(all_sites$Min_dist_river)
all_sites$Log_Fetch <- log(all_sites$Fetch)
all_sites$Log_Min_dist_port <- log(all_sites$Min_dist_port)
# 
# 
# 
# Mod_gam_PC1 <- gam(PC1 ~ s(Salinity, bs = "cs") + s(Log_Min_dist_river, bs = "cs") + s(Log_Fetch, bs = "cs") + s(Log_Min_dist_port, bs = "cs") + River_Size + Port_Status, method = "REML", family = "gaussian", data = all_sites )
# 
# library(DHARMa)
# simulateResiduals(Mod_gam_PC1, plot = T)
# 
# summary(Mod_gam_PC1)
# 
# draw(Mod_gam_PC1, residuals = T)
# 
# 
# Mod_gam_PC2 <- gam(PC2 ~ s(Salinity, bs = "cs") + s(Log_Min_dist_river, bs = "cs") + s(Log_Fetch, bs = "cs") + s(Log_Min_dist_port, bs = "cs") + River_Size + Port_Status, method = "REML", family = "gaussian", data = all_sites )
# 
# simulateResiduals(Mod_gam_PC2, plot = T)
# 
# summary(Mod_gam_PC2)
# 
# 
# draw(Mod_gam_PC2, residuals = T)
# 
################################################################################
# Assessment of abundance by total dencity of T-morphotype (Log_D_T) and E-morphotype (Log_D_E) 

# One common model

all_sites_long <-
all_sites %>% 
  dplyr::select(Site, Salinity, Log_Min_dist_river, Log_Fetch, Log_Min_dist_port, River_Size, Port_Status, Log_D_T, Log_D_E) %>% 
  rename(T_morphotype = Log_D_T, E_morphotype = Log_D_E) %>% 
  melt(id.vars = c("Site", "Salinity", "Log_Min_dist_river", "Log_Fetch", "Log_Min_dist_port", "River_Size", "Port_Status"), variable.name = "Morphotype", value.name = "Abundance")

all_sites_long$Morphotype <- factor(all_sites_long$Morphotype)


Mod_gam_Abundance <- gam(Abundance ~ s(Salinity, bs = "cs", by = Morphotype) + s(Log_Min_dist_river, bs = "cs", by = Morphotype) + s(Log_Fetch, bs = "cs", by = Morphotype) + s(Log_Min_dist_port, bs = "cs", by = Morphotype) + River_Size*Morphotype + Port_Status*Morphotype, method = "REML", family = "gaussian", data = all_sites_long )

# AIC(Mod_gam_Abundance)
# 
# all_sites_long_2 <- 
#   myt_full_reduced %>% 
#   mutate(Log_D_T = log(N_T * SQM_factor +1), Log_D_E = log(N_E*SQM_factor+1) ) %>% 
#   dplyr::select(Site, Position, Salinity, Log_Min_dist_river, Log_Fetch, Log_Min_dist_port, River_Size, Port_Status, Log_D_T, Log_D_E) %>% 
#    rename(T_morphotype = Log_D_T, E_morphotype = Log_D_E) %>% 
#   melt(id.vars = c("Site", "Position", "Salinity", "Log_Min_dist_river", "Log_Fetch", "Log_Min_dist_port", "River_Size", "Port_Status"), variable.name = "Morphotype", value.name = "Abundance")
#   
# all_sites_long_2$Morphotype <- factor(all_sites_long_2$Morphotype)


# 
# 
# Mod_gam_Abundanc_2 <- gam(Abundance ~ s(Salinity, bs = "cs", by = Morphotype) + s(Log_Min_dist_river, bs = "cs", by = Morphotype) + s(Log_Fetch, bs = "cs", by = Morphotype) + s(Log_Min_dist_port, bs = "cs", by = Morphotype) + River_Size*Morphotype + Port_Status*Morphotype + s(Site, bs = "re") + s(Position, bs = "re"), method = "REML", family = "gaussian", data = all_sites_long_2)
# 
# AIC(Mod_gam_Abundanc_2)
# 
# Mod_gam_Abundanc_3 <- gamm(Abundance ~ s(Salinity, bs = "cs", by = Morphotype) + s(Log_Min_dist_river, bs = "cs", by = Morphotype) + s(Log_Fetch, bs = "cs", by = Morphotype) + s(Log_Min_dist_port, bs = "cs", by = Morphotype) + River_Size*Morphotype + Port_Status*Morphotype, random = list(Site = ~1, Position = ~1), method = "REML", family = "gaussian", data = all_sites_long_2)
# 
# 
# AIC(Mod_gam_Abundanc_3$lme)


Mod_dummy2 <- lm(Abundance ~ Salinity + Log_Min_dist_river + Log_Fetch + Log_Min_dist_port + Morphotype + River_Size + Port_Status,  data = all_sites_long )

library(car)

VIF2 <- vif(Mod_dummy2) 

# appraise(Mod_gam_Abundance)
# library(DHARMa)
...
sm_abund <- 
  smooth_estimates(Mod_gam_Abundance) %>%
  add_confint()

sm_abund$.smooth <- gsub(pattern = ":MorphotypeE_morphotype", x =  sm_abund$.smooth, replacement = "")

sm_abund$.smooth <- gsub(pattern = ":MorphotypeT_morphotype", x =  sm_abund$.smooth, replacement = "")



```

```{r Visualization of Mod_gam_Abundance}
 
Pl_sal_smoother_abund <- 
sm_abund %>%
  filter(.smooth == "s(Salinity)") %>%
  ggplot() +
    geom_rug(aes(x = Salinity),
           data = myt_full,
           sides = "b", length = grid::unit(0.02, "npc")) +
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, x = Salinity, group = Morphotype),
              alpha = 0.2) +
  geom_line(aes(x = Salinity, y = .estimate, color = Morphotype), lwd = 1) +
  labs(y = "Partial effect", x = "Salinity") +
  geom_hline(yintercept = 0, linetype = 2) +
  theme_bw() +
...
sm_abund %>%
  ...
  ggplot() +
    geom_rug(aes(x = Log_Min_dist_river),
           data = myt_full,
           sides = "b", length = grid::unit(0.02, "npc")) +
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, x = Log_Min_dist_river, group = Morphotype),
              alpha = 0.2) +
  geom_line(aes(x = Log_Min_dist_river, y = .estimate, color = Morphotype), lwd = 1) +
  labs(y = "Partial effect", x = "Log Distance to the nearest river (km)") +
  geom_hline(yintercept = 0, linetype = 2) +
  theme_bw() +
  scale_color_manual(values = c("red", "blue")) +
...
sm_abund %>%
  ...
  ggplot() +
    geom_rug(aes(x = Log_Min_dist_port),
           data = myt_full,
           sides = "b", length = grid::unit(0.02, "npc")) +
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, x = Log_Min_dist_port, group = Morphotype),
              alpha = 0.2) +
  geom_line(aes(x = Log_Min_dist_port, y = .estimate, color = Morphotype), lwd = 1) +
  labs(y = "Partial effect", x = "Log Distance to the nearest port (km)") +
  geom_hline(yintercept = 0, linetype = 2) +
  theme_bw() +
  scale_color_manual(values = c("red", "blue")) +
...
sm_abund %>%
  ...
  ggplot() +
    geom_rug(aes(x = Log_Fetch),
           data = myt_full,
           sides = "b", length = grid::unit(0.02, "npc")) +
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, x = Log_Fetch, group = Morphotype),
              alpha = 0.2) +
  geom_line(aes(x = Log_Fetch, y = .estimate, color = Morphotype), lwd = 1) +
  labs(y = "Partial effect", x = "Log Fetch (km)") +
  geom_hline(yintercept = 0, linetype = 2) +
  theme_bw() +
  scale_color_manual(values = c("red", "blue")) +
...
```{r Parametric term visualization Abundance}

# Predictions for parametric terms 

ds_abund <-  data_slice(Mod_gam_Abundance, Morphotype = evenly(Morphotype),  Port_Status = evenly(Port_Status), River_Size = evenly(River_Size)) 


Fit_abund <- predict.gam(Mod_gam_Abundance, newdata = ds_abund, se = T, type = "link", exclude = c("s(Salinity)", "s(Log_Min_dist_river)",  "s(Log_Average_Fetch)",  "s(Log_Min_dist_port)", "s(Site)"))

ds_abund$Fit = Fit_abund$fit

ds_abund$SE = Fit_abund$se.fit
ds_abund$CI_low <- (Fit_abund$fit - 1.96*Fit_abund$se.fit)
ds_abund$CI_up <- (Fit_abund$fit + 1.96*Fit_abund$se.fit)


Pl_Small_abund <-
ds_abund %>% 
  filter(River_Size == "Small") %>%
ggplot(., aes(x = Morphotype, y = Fit, fill = Port_Status)) +
  geom_col(position = position_dodge()) +
  scale_fill_manual(values = c("blue", "gray")) +
  geom_errorbar(aes(ymin = CI_low, ymax = CI_up), width = 0.2, position = position_dodge(width = 0.9)) +
  theme_bw()+
...
```{r Combination of all figures with models predictions}
# plot_grid(Pl_sal_smoother, Pl_sal_smoother_abund, Pl_riv_dist_smoother, Pl_dist_riv_smoother_abund, Pl_port_dist_smoother, Pl_dist_port_smoother_abund, Pl_fetch_smoother,Pl_fetch_smoother_abund, Pl_Small_Pi, Pl_Small_abund, Pl_Large_Pi, Pl_Large_abund, ncol = 2)


```


```{r all smoothers in common figure }
sm_Ptros2 <-
  sm_Ptros %>% 
  select(".smooth", ".by", ".estimate", "Salinity", "Log_Min_dist_river", "Log_Fetch", "Log_Min_dist_port", ".lower_ci", ".upper_ci") %>% 
  mutate(Model = "Ptros", Morphotype = "Ptros")




melt(sm_Ptros2, id.vars = c(".smooth", ".by", ".estimate", ".lower_ci", ".upper_ci", "Model", "Morphotype")) %>% 
  filter(!is.na(value)) -> sm_Ptros_long
...
sm_abund <- 
  sm_abund %>%  
  select(".smooth", ".by", ".estimate", "Salinity", "Log_Min_dist_river", "Log_Fetch", "Log_Min_dist_port", ".lower_ci", ".upper_ci", "Morphotype") %>% 
  mutate(Model = "Abundance")

melt(sm_abund, id.vars = c(".smooth", ".by", ".estimate", ".lower_ci", ".upper_ci", "Model", "Morphotype")) %>% 
  filter(!is.na(value)) -> sm_abund_long


all_sm_long <- 
rbind(sm_Ptros_long, sm_abund_long)

all_sm_long$Morphotype <- factor(all_sm_long$Morphotype, levels = c("Ptros", "E_morphotype", "T_morphotype"), labels = c("Ptros", "E-morphotype", "T-morphotype") )


all_sm_long$Model <- factor(all_sm_long$Model, levels = c("Ptros", "Abundance"))

myt_full %>% 
  select(Site, Salinity, Log_Min_dist_river, Log_Fetch, Log_Min_dist_port) %>% 
  melt() %>% 
  group_by(Site, variable) %>% 
  summarise(value = mean(value)) -> predictors_rug

predictors_rug$variable <- factor(predictors_rug$variable, levels = c("Salinity", "Log_Min_dist_river", "Log_Min_dist_port", "Log_Fetch"))

predictors_rug$variable <- factor(predictors_rug$variable, labels = c("Salinity", "Log Distance to river", "Log Distance to port", "Log Fetch"))



all_sm_long$variable <- 
  factor(all_sm_long$variable, levels = c("Salinity", "Log_Min_dist_river", "Log_Min_dist_port", "Log_Fetch"))

all_sm_long$variable <- factor(all_sm_long$variable, labels = c("Salinity", "Log Distance to river", "Log Distance to port", "Log Fetch")) 
...
all_sm_long %>% 
  ggplot() +
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, x = value, fill = Morphotype),
              alpha = 0.2) +
  geom_line(aes(x = value, y = .estimate, color = Morphotype), lwd = 0.5) +
  ...
  geom_rug(data = predictors_rug, aes(x = value), lwd = 0.1) +
  theme(strip.background = element_blank(), strip.text = element_blank(), legend.position = "bottom") +
  labs(x = "", color = "") +
  theme(strip.placement = "outside", panel.grid = element_blank()) 
# +
#   ylim(-0.5, 0.5)
...
# ggsave(filename = "Pl_all_smoothers.svg", plot = Pl_all_smoothers, height = 7 , width = 7 * aspect_ratio)

# ggsave(filename = "Pl_all_smoothers_2.svg", plot = Pl_all_smoothers, height = 7 , width = 7 * aspect_ratio)
...
```{r}
ds %>% 
  select(Position, Port_Status, River_Size, Pi, Pi_CI_low, Pi_CI_up) %>% 
  mutate(Model = "Ptros") %>% 
  rename(Fit = Pi, OX = Position, CI_low = Pi_CI_low, CI_up = Pi_CI_up) -> ds_Pi 

ds_abund %>% 
  select(Morphotype, Port_Status, River_Size,Fit, CI_low, CI_up ) %>% 
  mutate(Model = "Abundance") %>% 
  rename(OX = Morphotype) -> ds_abundance

all_ds <- rbind(ds_Pi, ds_abundance)

all_ds$River_Size <- factor(all_ds$River_Size, labels = c("The nearest river is small", "The nearest river is large"))


Pl_Ptros_Pi <-
all_ds %>% 
  filter(Model == "Ptros") %>% 
  ggplot(aes(x = OX, y = Fit, fill = Port_Status)) +
  geom_col(position = position_dodge()) +
  facet_wrap(Model ~ River_Size, ncol = 2 ) + 
  geom_errorbar(aes(ymin = CI_low, ymax = CI_up), width = 0.2, position = position_dodge(width = 0.9), lwd = 0.1) +
  labs(x = "", y = "Partial effect") +
  guides(fill = "none") +
  scale_fill_manual(values = c("gray30", "gray") ) +
   theme(strip.background = element_blank(), strip.text = element_blank(), legend.position = "bottom", panel.grid = element_blank())



# ggsave(filename = "Pl_Ptros_Pi.svg", plot = Pl_Ptros_Pi, width = 3.5/2, height = 3.5/2)
...
all_ds %>% 
  ...
  ggplot(aes(x = OX, y = Fit, fill = Port_Status)) +
  geom_col(position = position_dodge()) +
  facet_grid(Model ~ River_Size) + 
  geom_errorbar(aes(ymin = CI_low, ymax = CI_up), width = 0.2, position = position_dodge(width = 0.9), lwd = 0.1) +
  guides(fill = "none") +
  labs(x = "", y = "Partial effect", fill = "The nearest port status:") +
  scale_fill_manual(values = c("gray30", "gray") ) +
  theme(strip.background = element_blank(), strip.text = element_blank(), legend.position = "bottom", panel.grid = element_blank()) +
  scale_y_continuous(labels = scales::number_format(accuracy = 0.1))

  
# ggsave(filename = "Pl_Abundance.svg", plot = Pl_Abundance, width = 3.5/2, height = 3.5/2)
...
```{r}
Pl_all_categorical <- 
  plot_grid(Pl_Ptros_Pi, Pl_Abundance, ncol = 1, align = "v")

# ggsave(filename = "Pl_all_categorical.svg", plot = Pl_all_categorical)
...
```{r Model2_output}
# Model 2 summary output preparation

Model_2_summ_smoothers <- tidy(Mod_gam_Abundance)

Model_2_summ_smoothers$term <- c(
  "s(Salinity) T-morphotype", 
  "s(Salinity) E-morphotype",
  "s(DistRiver) T-morphotype",
  "s(DistRiver) E-morphotype",
  "s(Fetch) T-morphotype",
  "s(Fetch) E-morphotype",
  "s(DistPort) T-morphotype",
  "s(DistPort) for E-morphotype")

Model_2_summ_smoothers[, 2:4] <- round(Model_2_summ_smoothers[, 2:4], 1)
Model_2_summ_smoothers[, 5] <- round(Model_2_summ_smoothers[, 5], 4)


Model_2_summ_parametric <- tidy(Mod_gam_Abundance, parametric = T)

Model_2_summ_parametric$term <- c("(Intercept)",
                                  "RiverSize(Large)",
                                  "Morphotype(E)",
                                  "PortStatus(Active)",
                                  "RiverSize(Large):Morphotype(E)",
                                  "PortStatus(Active):Morphotype(E)")
...
header <- c("Smoother terms", "Edf", "Ref.df", "F-statistic", "p-value")
...
header2 <- c("Parametric terms", "Parameter estimate", "SE", "t-statistic", "p-value")
...
```{r Model2_print_smoothers}
ft_Model_2_summ_smoothers <-
  ft_Model_2_summ_smoothers %>% 
  set_caption(caption = "Table 3. Parameters of smoothers and coefficients of parametric terms for Model 2 describing dependency of mussel abundance on environmental predictors. Edf – effective degrees of freedom.")
...
![Fig. 3. Predictions of the *Model 1* (upper row) and *Model 2* (lower row) describing the dependency of *Ptros* and T- and E-morphotype abundance, respecively, on (A) continous predictors (salinity,  distance to the nearest river, distance to the nearest port and fetch) and (B) discrete  factors (substrate type, nearest port status and size of the largest river). The  gray ribbons and wiskers demonstrate 95% confidence interval. The dotted lines in panels (A) represent the mean smoother value.](figures/Pl_all_smoothers_and_discrete_predictors_GAM.png)
...
Relationship of mussel abundance and environmental parameters evaluated by Model 2

**Model 2** explained 66% of the total deviance. It revealed a significant interactions between the  predictors in the model and the factor “Morphotype” (Table 3). The latter indicates asymmetry in the response of two morphotypes abundance to the influence of the environmental factors included in the model (the lower panel in Fig. 2 А, B). Thus, as salinity measured at low tide increased, there was a statistically significant increase in E-morphotype abundance, but T-morphotype showed no statistical relationship with this predictor. The abundance of E-morphotype  was independent of distance from the nearest port (*DistPort*), while T-morphotype  decreased in abundance significantly when the distance from port increased.  As the coastal openness (*Fetch*) grew, the abundance of E-morphotype mussels increased, while the T-morphotype mussels declined sharply at the highest levels of this predictor. The response of abundance to the status of the nearest port also showed an asymmetric trend: T-morphotype abundance was slightly higher at sites closer to active ports than to abandoned ports but the opposite trend was observed for E-morphotype mussels. The influence of the size of the nearest river on the abundance of mussels of the two morphotypes was not significant (the lower panel in Fig. 2 B). 

The only parameter to which the abundance of the two morphotypes was related in the same way was the distance to the nearest river (DistRiver). The abundance of mussels in both groups increased with distance from the river mouths (the lower panel in Fig. 2 A).

Finally, model 2 revealed higher abundance of E-morphotype mussels compared to T-morphotype (Table 3; the lower panel in  Fig. 2 B).
...
**Dependency of *Ptros* on substrate and mussel abundance evaluated by Model 3** 
...
# foo_mod <- lm(Dif_Ptros ~ Total_Ptros + Log_D_T + Log_D_E, data = all_sites)
# 
# vif(foo_mod)
...
# library(DHARMa)
# simulateResiduals(Mod_Dif, plot = T)
# 
# summary(Mod_Dif)

# draw(Mod_Dif, parametric = T, nrow = 1)

# draw(Mod_Dif, residuals = TRUE, parametric = T)
...
sm %>% 
  ...
  ggplot(., aes(x = Total_Ptros, y = .estimate)) +
...
sm %>% 
  ...
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci), alpha=0.2 ) +
  # geom_hline(yintercept = median(sm$est), linetype = 2) +
  geom_textbox(data = df_PC1, aes(x, y, label = label), width = grid::unit(0.7, "npc"), size = text_panel_size) +
  # geom_point(data = all_sites, aes(y = Residuals),cex = 1, colour = "steelblue3") +
  geom_hline(yintercept = 0, linetype = 2) +
...
sm %>% 
  ...
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci), alpha=0.2 ) +
  # geom_hline(yintercept = median(sm$est), linetype = 2)+
    geom_textbox(data = df_PC2, aes(x, y, label = label), width = grid::unit(0.7, "npc"), size = text_panel_size) +
  # geom_point(data = all_sites, aes(y = Residuals),cex = 1, colour = "steelblue3") +
  geom_hline(yintercept = 0, linetype = 2) +
...
ggsave(filename = "Fig_3.svg", plot = Pl_Model_3, width = 297,height = 210, units = "mm", dpi = 600)
...
# cor(Mod_gam_prediction_train, myt_site_substr$Ptros, method = "pearson")

# library(ppsr)
# 
# df <- data.frame(predicted = Mod_gam_prediction_train, target = myt_site_substr$Ptros)
# 
# score(df = df, x = "predicted", y = "target", algorithm = 'glm') [['pps']]




# NormalizedGini(Mod_gam_prediction_train, myt_site_substr$Ptros)
...
# cor(Mod_gam_prediction_test, myt_test_site$Ptros, method = "pearson")

# NormalizedGini(Mod_gam_prediction_test, myt_test_site$Ptros)

df <- data.frame(predicted = Mod_gam_prediction_test, target =  myt_test_site$Ptros)

score(df = df, x = "predicted", y = "target", algorithm = 'glm', cv_folds = 1) [['pps']]
...
# cor(Mod_gam_prediction_tuv, tuv$Ptros, method = "pearson")

# NormalizedGini(Mod_gam_prediction_tuv, tuv$Ptros)
...
# cor(Mod_gam_prediction_tuv2, tuv2$Ptros, method = "pearson")

# NormalizedGini(Mod_gam_prediction_tuv2, tuv2$Ptros)

# df <- data.frame(predicted = Mod_gam_prediction_tuv2, target =  tuv2$Ptros)
# 
# score(df = df, x = "predicted", y = "target", algorithm = 'glm', cv_folds = 1) [['pps']]
...
# cor(Mod_gam_prediction_tuv3, tuv3$Ptros, method = "pearson")

# NormalizedGini(Mod_gam_prediction_tuv3, tuv3$Ptros)

# df <- data.frame(predicted = Mod_gam_prediction_tuv3, target =  tuv3$Ptros)
# 
# score(df = df, x = "predicted", y = "target", algorithm = 'tree', cv_folds = 1) [['pps']]
...
Having applied the SDM approach to an unprecedentedly extensive material, we demonstrated that almost all environmental predictors considered in our study — namely, surf level, distance to the port, status of the port (active vs abandoned), salinity at low tide, size of the nearest river and fouling substrate (fucoid algae vs bottom substrates) — influenced the distribution of *Mytilus edulis* (*ME*) and *M. trossulus* (*MT*) in the White Sea. 

++++++++++++++
We revealed the influence of the environmental factors on both taxonomical structure of mixed populations and on abundance of two morphotypes, which are the proxies for mussel species.
++++++++++

The differences in the distribution, evident at scales ranging from meters to tens of kilometers, reflected the partial divergence of ecological niches of these two species. 
...
Our results show that the most expected habitat for *ME* in the White Sea littoral is a bottom substrate in a surf-exposed location with a “normal” surface salinity for the White Sea (24 ppt) situated away from ports and large rivers. The most expected habitat for *MT* is an algal substrate in a wind-protected location with a lower than “normal” salinity situated close to active ports. +++++ Both species tend to form abundant populations away from river mouth but proportion of *MT* tend to be higher if the nearest river is large. ++++ ~~and large rivers~~. Only the differences related to ports and substrates have been previously noted in the White Sea [@vainola2011mytilus; @katolikova2016genetic].
...
Additional point arrived when we assess the dependence of mussel abundance on salinity. A clear pattern of dependency was revealed for abundance of E-morphotype (i.e. *ME*) only which expectedly prefer higher salinity.  The abundance of T-morphotype (i.e. *MT*) appears to be less dependent on salinity, indicating broader ecological niche in relation to this environmental axis. 
...
The worst predictive value of the model for the Barents Sea data may be due to the following reasons. Firstly, considering that distribution of *ME* and *MT*  in the Tyuva Inlet by depth is non-random [@Marchenko2023], it may be associated with a large depth range of the sampling sites. The second reason may be a coarser categorization of the Barents Sea samples into algal and bottom ones. Since fouling substrate was not taken into account during sampling, we predicted it based on the projective cover of algae at the sampling site. Thirdly, we do not know whether the two species are non-randomly distributed across bottom and algal substrates in the sublittoral, where fucoids are replaced by kelps. ++++++ The fourth reason is associated with smaller sizes of the Tyuva inlet in comarison with the Kandalaksha Bay. In such a small inlet the variation in some predictors (*DistPort*, *DistRiver*, *Fetch*) is in narrow limits, which prevent the variation in predicted values (see **Fig. 4**).+++++
Finally, the fact that SDM tended to overestimate *Ptros* in the Barents Sea data (false positive predictions) is consistent with the observation that the proportion of *MT* has been declining in the study area in the 2010s under seemingly stable environmental conditions in terms of predictors included in our model [@Marchenko2023]. This observation suggests the presence of some yet unknown factors regulating the taxonomic structure.
