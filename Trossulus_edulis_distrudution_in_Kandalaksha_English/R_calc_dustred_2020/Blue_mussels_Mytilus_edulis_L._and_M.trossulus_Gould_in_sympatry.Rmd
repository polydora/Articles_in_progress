...
testSpatialAutocorrelation(simulationOutput, x = myt_full$Lon2, y = myt_full$Lat2)
...
testCategorical(simulationOutput, catPred = myt_full$Position)
...
qplot(myt_full$Ptros, E_pears) + geom_smooth() 

qplot(myt_full$Ptros, Pred_pi ) + geom_smooth() + geom_abline()

hist(abs(myt_full$Ptros-Pred_pi))
median(abs(myt_full$Ptros-Pred_pi))

mod_foo <- lm(Pred_pi ~ myt_full$Ptros)



qplot(myt_full$Prop_T, Pred_pi) + geom_smooth() + geom_abline()
