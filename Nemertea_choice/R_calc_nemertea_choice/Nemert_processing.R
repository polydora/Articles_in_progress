...
nem3 %>% ggplot(., aes(x = log(W + 1), y = P)) + geom_point() + geom_smooth(method = "lm")
 


model <- glmmTMB(cbind(N_in_signal, N_in_zero ) ~ log(W+1) + Signal_type  + (1|Exp),  data = nem3, family = betabinomial(link = "logit"))
