...
Посмотрите здесь внимательно. Я предлагаю дать вот такой рсунок вместо таблицы, которая приведена ниже. Идея в том, чтобы привести наиболее важные фенологические параметры (дата начала и конца) на одном рисунке с динамикой обилия. Кроме того здесь же можно привести значения угловых коэффициентов с оценками их значимости.
...
slopes_event$label <- with(slopes_event, paste(estimate, ", p = ", Mantel_P_value, sep = ""))
...
slopes_Abund2$label <- with(slopes_Abund2, paste(estimate, ", p = ", Mantel_P_value, sep = ""))
...
phen_cca_reduced <- ordistep(phen_cca, trace = FALSE, direction = "backward", scope = formula(phen_cca_0), permutations = how(nperm = 999))
...
```{r}
column_name <- c("Year", "Temperature peak date (JD)", "3 degrees date (JD)", "Summer start date (JD)", "Spring temperature (CD)", "Summer temperture (CD)", "Ice clear date (JD)", "Summer start date in previous year (JD)", "Summer duriation in prev. year (JD)", "Summer end date in prev. year (JD)", "Spring temperature in prev.year (CD)", "Summer temperature in prev. year (CD)", "NAO", "NAO in prev.year", "AOI", "AOI in prev. year"  )

env_print <- env
env_print[ , c(2:4, 7:10)] <- round(env_print[ , c(2:4, 7:10)], 0)
env_print [,5] <- round(env_print [,5], 1) 
env_print [,11:12] <- round(env_print [,11:12], 1) 
env_print [,13:14] <- round(env_print [,13:14], 3) 
