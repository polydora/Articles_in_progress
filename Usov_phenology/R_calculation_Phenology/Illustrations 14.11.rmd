...
Pl_phen1 <- Pl_phen_begin + geom_text(data = slopes_event [slopes_event$Event == "Begin", ], aes(label = label, x = 1961, y = max + 5), size = 2, hjust = "inward") + scale_y_continuous(breaks=c(50,100, 150, 200, 250, 300, 350))

Pl_phen2 <- Pl_phen_end + geom_text(data = slopes_event[slopes_event$Event == "End", ], aes(label = label, x = 1961, y = max + 5), size = 2, hjust = "inward") + scale_y_continuous(breaks=c(50,100, 150, 200, 250, 300, 350))


Pl_abund2 <- Pl_abund + geom_text(data = slopes_Abund3, aes(label = label, x = 1961, y = max + 0.05*max), size = 2, hjust = "inward") + labs(y = bquote('Abundance ('~ind ~m^-3 ~')')) + theme(strip.text = element_text(size = 10))
...
Pl_factors2 + theme(strip.text = element_text(size = 10))
...
ggplot(gg_phen_cca_reduced[gg_phen_cca_reduced$Score == "species", ],  aes(x = CCA1, y = CCA2))  +
  ...
  geom_text_repel(data = gg_phen_cca_reduced[gg_phen_cca_reduced$Score == "biplot" & gg_phen_cca_reduced$Label %in% signif_term,  ], aes(x =  CCA1, y = CCA2, label = Label), size = 4, point.padding = 0.2) +
