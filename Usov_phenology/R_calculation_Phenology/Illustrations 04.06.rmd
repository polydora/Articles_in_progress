...
Y_Begin <-  phen_all[,c("Species","Begin")] %>% group_by(Species) %>% summarise(max = max(Begin))

Y_Begin$Event <- "Begin"


Y_End <-  phen_all[,c("Species","End")] %>% group_by(Species) %>% summarise(max = max(End))
...
Pl_phen1 <- Pl_phen_begin + geom_text(data = slopes_event [slopes_event$Event == "Begin", ], aes(label = label, x = 1963, y = max + 5), size = 2, hjust = "inward")

Pl_phen2 <- Pl_phen_end + geom_text(data = slopes_event[slopes_event$Event == "End", ], aes(label = label, x = 1963, y = max + 5), size = 2, hjust = "inward")
