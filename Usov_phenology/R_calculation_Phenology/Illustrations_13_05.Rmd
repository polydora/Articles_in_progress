...
####################3
column_name <- c("Year", "TPD", "SpSD", "SuSD", "SuFD", "ICD", "SuSD in previous year",     "SpT", "SuT",  , "SuDur in prev. year", "SuED in prev. year", "SpT in prev.year", "SuT in prev. year", "NAO", "NAO in prev.year", "AOI", "AOI in prev. year"  )
...
env_print_short <- env_print[,c(2:4, 7)]

names(env_print_short) <- c("TPD", "SpSD", "SuSD", "ICD")

env_print_short_melt <- melt(env_print_short, variable.name = "Event", value.name = "Day")

env_print_short_melt$Event <- factor(env_print_short_melt$Event, levels = c("ICD","SpSD", "SuSD", "TPD" ))
...
- SuSD -- Summer start date (the day when the water temperature overcomes 10 Celsium degrees)     
