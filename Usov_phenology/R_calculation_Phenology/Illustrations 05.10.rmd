...
Всего было рассмотрено 16 гидролого-климатических предикторов, 7 величин, описывающих биотические параметры (логарифмы обилия видов в данном году), а также год, как непрерывный предиктор, характеризующий линейный тренд в многолетних изменениях. Однако часть предикторов продемонстрировало высокую степень коллинеарности (взаимозависимости) поэтому их количество было сокращено (были удалены предикторы демонстрирующие VIF > 5). Оставшиеся предикторы были включены в модель канонического корреспондентного анализа. 
 
**Коля и Даша! обратите внимание, что в этой верси я подумал и решил, что не будем редуцировать модель. Пусть будет все, что не коллинеарно. Мы отбракуем лишнее за счет значимости присутствия предиктора в мдели**
=======
...
<<<<<<< Updated upstream
  ...
  geom_text_repel(data = gg_phen_cca_reduced[gg_phen_cca_reduced$Score == "biplot" & gg_phen_cca_reduced$Label %in% signif_term,  ], aes(x =  CCA1, y = CCA2, label = Label), size = 4) +
=======
