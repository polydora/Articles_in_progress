...
Pl
ggsave(filename = paste("SEM_Figures/Catch_efforts.svg", sep = ""))
...
species <- wintering_data$Sci_Name

# Преобразование названий
species_modified <- sapply(species, function(x) {
  # Разделяем на род и видовой эпитет
  parts <- strsplit(x, " ")[[1]]
  genus <- parts[1]
  epithet <- parts[2]
  
  # Проверяем, начинается ли род с "Ph"
  if (grepl("^Ph", genus, ignore.case = TRUE)) {
    genus_abbr <- "Ph."
  } else {
    # Берем только первую букву и добавляем точку
    genus_abbr <- paste0(substr(genus, 1, 1), ".")
  }
  
  # Собираем обратно
  return(paste(genus_abbr, epithet))
}, USE.NAMES = FALSE)


species_modified <- gsub(pattern = " ", replacement = "", x = species_modified)

wintering_data$Sci_Name_full <- species_modified
...
ggplot(wintering_data, aes(x = Lon, y = CCA2)) +
  geom_point( size = 3) +
  geom_text_repel(aes(label = Sci_Name_full)) +
  scale_color_gradient(high = "red", low = "yellow") +
...
ggsave(filename = "Figure_2.svg", plot = Pl_mod_predict)
