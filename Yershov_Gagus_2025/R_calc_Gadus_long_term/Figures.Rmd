---
title: ''
output: html_document
---

```{r setup, include=FALSE}
library(knitr)
opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE)
Sys.setlocale("LC_TIME", "en_US.UTF-8")
```


```{r Пакеты}
library(dplyr)
library(ggplot2)
library(readxl)
library(flextable)
library(lubridate)
library(mgcv)
library(gratia)
library(cowplot)
library(reshape2)


theme_set(theme_bw() + 
          theme(axis.text = element_text(family = "sans", size = 7)))
```


```{r}
gadus <- read_excel("Data/ТРЕСКА_LONG-TERM_CHANGES.xlsx")

gadus <-
gadus %>% 
  mutate(Date2 = as.POSIXct(as.Date(Date2, format = "%d.%m.%Y")), 
         DOY = yday(Date2), Month = month(Date2, label = TRUE), DOM = day(Date2)) 



```


## Размерная структура



```{r}
gadus <-
  gadus %>% 
  filter(Year != 2016)
```


<!-- ```{r} -->
<!-- gadus %>%  -->
<!--   group_by(Year) %>%  -->
<!--   summarise(Mean_L = round(mean(L), 1), SD = round(sd(L), 2), Min = min(L), Max = max(L)) %>%  -->
<!--   kable() -->
<!-- ``` -->


<!-- По всему материалу -->

<!-- ```{r} -->
<!-- gadus %>%  -->
<!--   summarise(Mean_L = round(mean(L), 1), SD = round(sd(L), 2), Min = min(L), Max =max(L)) %>%  -->
<!--   kable() -->

<!-- ``` -->



<!-- Модель для описания многолетней и сезонной динамики размеров  -->


```{r echo=FALSE}
Model_L <-  gam(L ~ s(Year) + s(DOY, k = 8), data = gadus)
```



```{r}
# summary(Model_L)
library(flextable)
library(broom)

ft <-
tidy(Model_L) %>% 
  flextable() %>%
  colformat_double(j = c(2, 3, 4), digits = 1) %>% 
  colformat_double(j = 5, digits = 3) %>% 
  set_header_labels(values = c("Smoother term", "edf", "ref.df", "F", "p-value" )) 
  
# ft
# draw(Model_L)
```


```{r}
sm <- smooth_estimates(Model_L) |>
  add_confint()

sm %>% 
  filter(.smooth == "s(Year)") ->
  smoother_year

sm %>% 
  filter(.smooth == "s(DOY)") ->
  smoother_doy

```




```{r}
# gadus %>% 
#   ggplot(aes(x = Year, y = L)) +
#   geom_boxplot(aes(group = Year)) +
#   geom_hline(yintercept = mean(gadus$L), linetype = 2) +
#    scale_y_continuous(breaks = seq(0, max(gadus$L), by = 5)) +
#   geom_line(data = smoother_year, aes(x = Year, y = .estimate + mean(gadus$L))))
#   
Pl_L_boxplot <-
gadus %>% 
  ggplot() +
  geom_boxplot(aes(x = Year, y = L, group = Year)) +
  geom_ribbon(data = smoother_year, aes(x = Year, ymin = .lower_ci + mean(gadus$L), ymax = .upper_ci + mean(gadus$L)), alpha = 0.8, fill = "gray") + 
  geom_line(data = smoother_year, aes(x = Year, y = .estimate + mean(gadus$L)),
            color = "blue", linewidth = 1) +
  geom_hline(yintercept = mean(gadus$L), linetype = 2) +
  scale_y_continuous(breaks = seq(0, max(gadus$L), by = 15)) +
  labs(x = "Year", y = "Length, cm") 
```

```{r}
Pl_L_hist <-
  gadus %>% 
  ggplot(aes(x = L)) +
  geom_histogram(aes(y = after_stat(density * 100)), binwidth = 1) +
  labs(x = "Length, cm", y = "Proportion, %") +
  scale_x_continuous(breaks = seq(0, max(gadus$L), by = 5)) 
  
```


```{r}
Pl_doy <- 
  ggplot() +
  geom_ribbon(data = smoother_doy, aes(x = DOY, ymin = .lower_ci + mean(gadus$L), ymax = .upper_ci + mean(gadus$L)), alpha = 0.8, fill = "gray") + 
  geom_line(data = smoother_doy, aes(x = DOY, y = .estimate + mean(gadus$L)),
            color = "blue", linewidth = 1) +
  geom_hline(yintercept = mean(gadus$L), linetype = 2) +
    labs(x = "DOY", y = "Length, cm")
  
```

```{r}
library(patchwork)
Fig_2 <- 
Pl_L_hist/(Pl_L_boxplot + Pl_doy) +
  plot_annotation(tag_levels = "A")

Fig_2
```

```{r}
ggsave(filename = "Figures/Fig_2.tif", plot = Fig_2, dpi = 1200)
```



# Общее описание рациона

```{r}

# Отбираем только тех рыб, у которых была пища в желудках
gadus %>% 
  select(-Date, -W, - Date2) %>% 
  melt(id.vars = c("Id", "Year", "DOY","Month", "DOM",  "Sex", "L"), variable.name = "Species", value.name = "Weight") ->
  gadus_long 

gadus_long %>% 
  group_by(Id) %>% 
  summarise(Weight_total = sum(Weight)) %>% 
  filter(Weight_total == 0) %>% 
  pull(Id) ->
  empty_stomack


gadus_long <-
  gadus_long %>% 
  mutate(Decade = case_when(DOM %in% 1:10 ~ 1,
                            DOM %in% 11:20~ 2, 
                            DOM %in% 21:31 ~ 3))
```




<!-- ## Таблица с характеристиками питания -->


<!-- Средний вес в пищевом комке -->

<!-- ```{r} -->
<!-- gadus_long  %>%  -->
<!--   group_by(Species, Year) %>%  -->
<!--   summarise(Mean_Weight = round(mean(Weight),2) ) %>%  -->
<!--   dcast(formula = Species ~ Year) -> -->
<!--   mean_weight -->
<!-- ``` -->






<!-- Встречаемость (%) -->

```{r}
gadus_long  %>% 
  group_by(Species, Year) %>% 
  summarise(Freq = round(sum(Weight !=0)/n(), 3) *100 ) %>% 
  dcast(formula = Species ~ Year) ->
  freq
```







```{r}
sci_name <- read_excel("Data/ТРЕСКА_LONG-TERM_CHANGES.xlsx", sheet = "Species")

sci_name$Group <- factor(sci_name$Group, levels = c( "Polychaeta", "Crustacea", "Ophiuroidea", "Holothuroidea", "Pisces"))

gadus_long %>% 
  filter(!Id %in% empty_stomack) %>% 
  group_by(Species) %>% 
  summarise(Freq = round(sum(Weight !=0)/length(unique(Id)), 4) *100,  Sum_Weight = sum(Weight)) %>% 
  mutate(Prop =  round(Sum_Weight/sum(.$Sum_Weight)*100, 2)) %>% 
  merge(., sci_name) %>% 
  select(Group, Sci_name, Freq, Prop ) %>% 
  arrange(Group) %>% 
  mutate(Freq = ifelse(round(Prop, 1) <= 0.1, "+", Freq), Prop = ifelse(round(Prop, 1) <= 0.1, "+", Prop)) %>% 
  select(-Group) %>% 
  flextable() %>% 
  set_header_labels(values = c("Species", "Frequency", "Weight proportion")) ->
  ft
  
# ft %>% 
  # autofit()

```

```{r}
dff <- 
gadus_long %>% 
  filter(!Id %in% empty_stomack) %>% 
  group_by(Species) %>% 
  summarise(Freq = round(sum(Weight !=0)/length(unique(Id)), 4) *100,  Sum_Weight = sum(Weight)) %>% 
  mutate(Prop =  round(Sum_Weight/sum(.$Sum_Weight)*100, 2)) %>% 
  merge(., sci_name) %>% 
  select(Group, Sci_name, Freq, Prop ) %>% 
  arrange(Group) 

# cor.test(dff$Freq, dff$Prop, method = "spearman")

```


<!-- Таблица с описанием долей для отдельных групп -->

```{r}
gadus_long %>% 
  filter(!Id %in% empty_stomack) %>%
  merge(., sci_name) %>%
  group_by(Id, Group) %>% 
  summarise(Occur = sum(Weight !=0), Weight = sum(Weight)) %>% 
  group_by(Group) %>% 
  summarise(Freq = round(sum(Occur !=0)/length(unique(Id)), 4) *100,  Sum_Weight = sum(Weight)) %>% 
  mutate(Prop =  round(Sum_Weight/sum(.$Sum_Weight)*100, 2)) %>% 
  select(Group, Freq, Prop ) %>% 
  arrange(Group) %>% 
  flextable() %>% 
  set_header_labels(values = c("Group", "Frequency", "Weight proportion")) ->
  ft
  
# ft %>% 
#   autofit()

```




## Динамика пустых желудков

```{r}
gadus_long %>% 
  group_by(Id, L, Year, DOY) %>% 
  summarise(Total_Weight = sum(Weight)) ->
  total_weight 

```






```{r}

total_weight %>% 
  mutate(Empty = ifelse(Total_Weight == 0, 1, 0)) -> 
  total_weight

total_weight <-
total_weight %>% 
  filter(Year != 2016)

```

```{r}
Mod_Empty <- gam(Empty ~ s(Year) + s(DOY) + s(L), data = total_weight, family = "binomial")

# summary(Mod_Empty)

```


```{r}
ft <-
tidy(Mod_Empty) %>% 
  flextable() %>%
  colformat_double(j = c(2, 3, 4), digits = 1) %>% 
  colformat_double(j = 5, digits = 3) %>% 
  set_header_labels(values = c("Smoother term", "edf", "ref.df", "Chi.sq", "p-value" )) 
  
# ft
# draw(Model_L)
```


```{r}
sm <- smooth_estimates(Mod_Empty) |>
  add_confint()

sm %>% 
  filter(.smooth == "s(Year)") ->
  smoother_year

sm %>% 
  filter(.smooth == "s(DOY)") ->
  smoother_doy

```







```{r}
Pl_doy <- 
  ggplot() +
  geom_ribbon(data = smoother_doy, aes(x = DOY, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.8, fill = "gray") + 
  geom_line(data = smoother_doy, aes(x = DOY, y = .estimate),
            color = "blue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_rug(data = total_weight, aes(x = DOY) ) +
  # ggtitle("Model prediction for \nseasonal dynamics") +
  labs(y = "Partial effect") +
  annotate(geom = "segment",y = -1.6, yend = -1.6, x = 172, xend = 182, linewidth = 1, color = "gray80") +
  annotate(geom = "text", x = 177, y =-1.45, label = "June")+
  annotate(geom = "segment",y = -1.6, yend = -1.6, x = 183, xend = 213, linewidth = 1, color = "gray60") +
  annotate(geom = "text", x = 198, y =-1.45, label = "July") +
  annotate(geom = "segment",y = -1.6, yend = -1.6, x = 214, xend = 231, linewidth = 1, color = "gray40") +
  annotate(geom = "text", x = 222, y =-1.45, label = "August") 

  
```


```{r}
Pl_year <- 
  ggplot() +
  geom_ribbon(data = smoother_year, aes(x = Year, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.8, fill = "gray") + 
  geom_line(data = smoother_year, aes(x = Year, y = .estimate),
            color = "blue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_rug(data = total_weight, aes(x = Year) ) +
  # ggtitle("Model prediction for \nlong-term dynamics") +
  labs(y = "Partial effect")
```


```{r}
Fig_3 <-
Pl_year + Pl_doy + plot_annotation(tag_levels = "A")

Fig_3
```


```{r}
ggsave(filename = "Figures/Fig_3.tif", plot = Fig_3, dpi = 1200)
```


## Динамика веса пищевого комка в заполненных желудках


```{r}
total_weight %>% 
  mutate(Empty = ifelse(Total_Weight == 0, 1, 0)) %>% 
  filter(Empty == 0) -> 
  total_weight_full

```

```{r}
Mod_Total_Weight <- gam(Total_Weight ~ s(Year) + s(DOY) + s(L, k = 5), data = total_weight_full)

# summary(Mod_Total_Weight)
```



```{r}
ft <-
tidy(Mod_Total_Weight) %>% 
  flextable() %>%
  colformat_double(j = c(2, 3, 4), digits = 1) %>% 
  colformat_double(j = 5, digits = 3) %>% 
  set_header_labels(values = c("Smoother term", "edf", "ref.df", "F", "p-value" )) 
  
# ft
# draw(Model_L)
```


```{r}
sm <- smooth_estimates(Mod_Total_Weight) |>
  add_confint()

sm %>% 
  filter(.smooth == "s(Year)") ->
  smoother_year

sm %>% 
  filter(.smooth == "s(L)") ->
  smoother_L

```


```{r}
Pl_year <- 
  ggplot() +
  geom_ribbon(data = smoother_year, aes(x = Year, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.8, fill = "gray") + 
  geom_line(data = smoother_year, aes(x = Year, y = .estimate),
            color = "blue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_rug(data = total_weight, aes(x = Year) ) +
  # ggtitle("Model prediction for \nlong-term dynamics") +
  labs(y = "Partial effect")
```



```{r}
Pl_L <- 
  ggplot() +
  geom_ribbon(data = smoother_L, aes(x = L, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.8, fill = "gray") + 
  geom_line(data = smoother_L, aes(x = L, y = .estimate),
            color = "blue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_rug(data = total_weight, aes(x = L) ) +
  # ggtitle("Model prediction for \nbody size") +
  labs(y = "Partial effect", x = "Cod length, cm") +
  xlim(20, 75)
```

```{r}
Fig_4 <- 
Pl_year + Pl_L + plot_annotation(tag_levels = "A")

Fig_4
```


```{r}
ggsave(filename = "Figures/Fig_4.tif", plot = Fig_4, dpi = 1200)
```




```{r}
gadus_long <-
merge(gadus_long, total_weight_full)


gadus_long %>%  
  select(-Empty) %>% 
  mutate(Prop = Weight/Total_Weight) -> 
  gadus_long

gadus_long %>% 
  mutate(Presence = ifelse(Prop == 0, 0, 1)) ->
  gadus_long

gadus_long <- 
  gadus_long %>% 
  filter(Year != 2016)

```


## Динамика встречаемости пищевых объектов


```{r}
gadus_long %>% 
  group_by(Species) %>% 
  summarise(Freq = mean(Presence), Mean_Prop = mean(Prop)) %>% 
  arrange(desc(Freq)) ->
  gadus_total_means

```


```{r}
Freq_threshould <- 0.05
```


```{r}
gadus_total_means %>% 
  filter(Freq >= Freq_threshould) %>% 
  pull(Species) ->
  selected_Species
```




```{r}
abund_species <- 
  gadus_long %>% 
  filter(Species %in% selected_Species)

abund_species$Species <- factor(abund_species$Species)

abund_species$Species <- factor(abund_species$Species, labels = c("G.aculeatus", "C.pallasii marisalbi", "Fish eggs", "O.aculeata", "A.virens + N.pelagica" ))
```


```{r}

mod_freq <- gam(Presence ~ s(Year, by = Species, k =5) + s(DOY, by = Species, k =5) + s(L, by = Species, k =5) + Species, data = abund_species, family = "binomial")

# summary(mod_freq)
```





```{r}
ft <-
tidy(mod_freq) %>% 
  flextable() %>%
  colformat_double(j = c(2, 3, 4), digits = 1) %>% 
  colformat_double(j = 5, digits = 3) %>% 
  set_header_labels(values = c("Smoother term", "edf", "ref.df", "Chi.sq", "p-value" )) 
  
# ft
# draw(Model_L)
```



```{r}
sm <- smooth_estimates(mod_freq) |>
  add_confint()

sm %>% 
  filter(if_any(everything(), ~ grepl("s\\(Year\\)", .))) ->
  smoother_year

sm %>% 
  filter(if_any(everything(), ~ grepl("s\\(DOY\\)", .))) ->
  smoother_doy


sm %>% 
  filter(if_any(everything(), ~ grepl("s\\(L\\)", .))) ->
  smoother_L

```



```{r}
Pl_L_species <- 
  ggplot() +
  geom_ribbon(data = smoother_L, aes(x = L, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.8, fill = "gray") + 
  geom_line(data = smoother_L, aes(x = L, y = .estimate),
            color = "blue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_rug(data = total_weight, aes(x = L) ) +
  # ggtitle("Body size") +
  labs(y = "Partial effect", x = "Cod length, cm") +
  facet_wrap(~Species, ncol = 1) +
  xlim(20, 75)
```

```{r}
Pl_year_species <- 
  ggplot() +
  geom_ribbon(data = smoother_year, aes(x = Year, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.8, fill = "gray") + 
  geom_line(data = smoother_year, aes(x = Year, y = .estimate),
            color = "blue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_rug(data = total_weight, aes(x = Year) ) +
  # ggtitle("Long-term dynamics") +
  labs(y = "Partial effect")+
  facet_wrap(~Species, ncol = 1) 

```


```{r}
Pl_doy_species <- 
  ggplot() +
  geom_ribbon(data = smoother_doy, aes(x = DOY, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.8, fill = "gray") + 
  geom_line(data = smoother_doy, aes(x = DOY, y = .estimate),
            color = "blue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_rug(data = total_weight, aes(x = DOY) ) +
  # ggtitle("Seasonal dynamics") +
  labs(y = "Partial effect") + 
  annotate(geom = "segment",y = -5, yend = -5, x = 172, xend = 182, linewidth = 1, color = "gray80") +
  annotate(geom = "text", x = 177, y =-4.5, label = "June")+
  annotate(geom = "segment",y = -5, yend = -5, x = 183, xend = 213, linewidth = 1, color = "gray60") +
  annotate(geom = "text", x = 198, y =-4.5, label = "July") +
  annotate(geom = "segment",y = -5, yend = -5, x = 214, xend = 231, linewidth = 1, color = "gray40") +
  annotate(geom = "text", x = 222, y =-4.5, label = "August") +
  facet_wrap(~Species, ncol = 1) 


```


<!-- ```{r} -->
<!-- Pl_L_species -->
<!-- ``` -->



<!-- ```{r} -->
<!-- Pl_doy_species -->
<!-- ``` -->


<!-- ```{r} -->
<!-- Pl_year_species -->
<!-- ``` -->

<!-- ## Динамика весовой доли -->


<!-- (At!) Для каждого вида корма была найдена сумма его веса за каждый день и отнесена к сумме весов всех пищевых комков, отловленных в каждый из дней.  -->
<!-- Подобрать модель, в которой не нарушались бы условия применимости регрессионного анализа на этом материале не удается. Поэтому просто приводятся первичные данные. Для отражения общего направления динамики приведена линия тренда, подобранная методом "loess". Поскольку строго статистического анализа не проводилось, то доверительные интервалы не приводятся.  -->



```{r}
abund_species %>%
  group_by(DOY, Species) %>% 
  summarise(Tot_Weight = sum(Weight)) ->
  df_food_species
  
abund_species %>% 
  group_by(Id, DOY) %>% 
  summarise(Food_weight = mean(Total_Weight)) %>% 
  group_by(DOY) %>% 
  summarise(Food_weight = sum(Food_weight)) ->
  df_all_food

df_food_species_prop <- 
merge(df_food_species, df_all_food) %>% 
  mutate(Prop = Tot_Weight/Food_weight)

Pl_weight_doy <- 
df_food_species_prop %>%
  ggplot(aes(x = DOY, y = Prop)) +
  geom_point(color = "gray40", size = 2) +
  # geom_smooth(method = "gam", se = F, method.args = list(family = betar())) +
  facet_wrap(~Species, scales = "free_y", ncol = 1) +
  labs(y = "Weight proportion") +
  annotate(geom = "segment",y = 0, yend = 0, x = 172, xend = 182, linewidth = 1, color = "gray80") +
  annotate(geom = "text", x = 177, y =5.5/100, label = "June")+
  annotate(geom = "segment",y = 0, yend = 0, x = 183, xend = 213, linewidth = 1, color = "gray60") +
  annotate(geom = "text", x = 198, y =5.5/100, label = "July") +
  annotate(geom = "segment",y = 0, yend = 0, x = 214, xend = 231, linewidth = 1, color = "gray40") +
  annotate(geom = "text", x = 222, y =5.5/100, label = "August") +
  facet_wrap(~Species, ncol = 1) +
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 10)  # Проценты с точностью до 1%
  )


```



<!-- Аналогично для многолетних изменений -->

<!-- (At!) Для каждого вида корма была найдена сумма его веса за каждый год и отнесена к сумме весов всех пищевых комков, отловленных в каждый год.  -->
<!-- Подобрать модель, в которой не нарушались бы условия применимости регрессионного анализа на этом материале тоже не удается. Поэтому просто приводятся первичные данные. Для отражения общего направления динамики приведена линия тренда, подобранная методом "loess". Поскольку строго статистического анализа не проводилось, то доверительные интервалы не приводятся.  -->





```{r}
abund_species %>%
  group_by(Year, Species) %>% 
  summarise(Tot_Weight = sum(Weight)) ->
  df_food_species
  
abund_species %>% 
  group_by(Id, Year) %>% 
  summarise(Food_weight = mean(Total_Weight)) %>% 
  group_by(Year) %>% 
  summarise(Food_weight = sum(Food_weight)) ->
  df_all_food

df_food_species_prop <- 
merge(df_food_species, df_all_food) %>% 
  mutate(Prop = Tot_Weight/Food_weight)


Pl_weight_year <-
df_food_species_prop %>%
  ggplot(aes(x = Year, y = Prop)) +
  geom_line() +
  geom_point(color = "gray40") +
  # geom_smooth(method = "gam", se = F, method.args = list(family = betar())) +
  facet_wrap(~Species) +
  labs(y = "Weight proportion") +
  facet_wrap(~Species, ncol = 1) +
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 10)  # Проценты с точностью до 1%
  )
  

```

<!-- Предлагаю разделить рисунки иначе. -->

<!-- Этот рисунок иллюстрирует все про Модель 5. -->

<!-- ```{r, fig.width=15, fig.height=15} -->
<!-- plot_grid(Pl_L_species, Pl_doy_species, Pl_year_species, ncol = 3, labels = "AUTO") -->
<!-- ``` -->





```{r, fig.width=15, fig.height=15}
Fig_5 <- 
Pl_L_species +
  facet_wrap(~Species, ncol = 3)

Fig_5
```

```{r}
ggsave(filename = "Figures/Fig_5.tif", plot = Fig_5, dpi = 1200)
```




```{r, fig.width=15, fig.height=15}
Fig_6 <-
Pl_doy_species +
  facet_wrap(~Species, ncol = 3)

Fig_6
```

```{r}
ggsave(filename = "Figures/Fig_6.tif", plot = Fig_6, dpi = 1200)

```






```{r, fig.width=15, fig.height=15}
Fig_7 <- 
Pl_year_species +
  facet_wrap(~Species, ncol = 3)

Fig_7
```


```{r}
ggsave(filename = "Figures/Fig_7.tif", plot = Fig_7, dpi = 1200)

```



<!-- А этот рисунок отдельно описывает поведение весовой доли. -->


```{r}
gadus_long <-
  gadus_long %>% 
  mutate(Species2 = case_when(Species %in% selected_Species ~ Species,
                              !Species %in% selected_Species ~ "Other"))

```




```{r}

  gadus_long$Species2 <- factor(gadus_long$Species2, levels = c("Other", "O.aculeata", "Nereis_spp.",  "Fish_eggs", "C.pallasii_marisalbi", "G.aculeatus"), labels = c("Other food", "O.aculeata", "A.virens+N.pelagica",  "Fish eggs", "C.pallasii marisalbi", "G.aculeatus"))
 

```




```{r}
gadus_long %>% 
  group_by(Month, Decade, Species2) %>% 
  summarise(Sum_weight = sum(Weight)) -> 
  species_weight_decade 


gadus_long %>% 
  group_by(Month, Decade) %>% 
  summarise(Total_weight = sum(Weight)) -> 
  total_weight_decade 

prop_weight_decade <-
merge(species_weight_decade, total_weight_decade) %>% 
  mutate(Month_Decade = factor(paste(Month, Decade, sep = "_")))%>% 
  mutate(Prop_decade = Sum_weight / Total_weight)


prop_weight_decade$Month_Decade <- factor(prop_weight_decade$Month_Decade, levels =  c("Jun_3", "Jul_1", "Jul_2", "Jul_3", "Aug_1", "Aug_2"), labels = c("late June", "early July", "mid July", "late July", "early August", "mid August"))

Pl_weight_doy_color <-
prop_weight_decade %>% 
 ggplot(aes(x = (Month_Decade), y = Prop_decade, fill = Species2)) +
  geom_col()+
  labs(x = "Decade of Month", y = "Proportion of the diet by weight, %", fill = "")


```



```{r}
library(ggpattern)

Pl_weight_doy <- 
  prop_weight_decade %>% 
  ggplot(aes(x = Month_Decade, y = Prop_decade)) +
  geom_col_pattern(
    aes(
      fill = Species2,
      pattern = Species2,
      pattern_angle = Species2
      ),
    color = "black",
    # key_glyph = "polygon_pattern"
  ) +
  labs(x = "Decade of Month", 
       y = "Proportion of the diet by weight") +
  theme_bw() +
  theme(legend.position = "bottom")

# Настройка параметров:
# 1. Тип узора: для первых трёх - "none" (сплошная заливка), для 4 и 5 - "stripe", для 6 - "stipple"
Pl_weight_doy <- Pl_weight_doy +
  scale_pattern_manual(
  values = c("stripe", "stripe", "none", "none", "none", "none"),
  name = "Species"
) +
  scale_fill_manual(values = c("white", "white", "white", "gray80", "gray40", "black" ), name = "Species")


# 3. Угол наклона: для 4-го - 45° (влево), для 5-го - -45° (вправо), для остальных неважно
Pl_weight_doy <- Pl_weight_doy +
  scale_pattern_angle_manual(
    values = c(-45, 45, 0, 0, 0, 0),
    name = "Species"
  )

# 4. Объединяем легенды
Pl_weight_doy <- Pl_weight_doy +
  guides(
    pattern = guide_legend(title = "Species"),
    pattern_fill = guide_legend(title = "Species"),
    pattern_angle = guide_legend(title = "Species")
  )

library(scales)

Pl_weight_doy <-
Pl_weight_doy +
  scale_y_continuous(labels = scales::percent)

```



```{r}
gadus_long %>% 
  group_by(Year, Species2) %>% 
  summarise(Sum_weight = sum(Weight)) -> 
  species_weight_year 


gadus_long %>% 
  group_by(Year) %>% 
  summarise(Total_weight = sum(Weight)) -> 
  total_weight_year 

prop_weight_year <-
merge(species_weight_year, total_weight_year) %>% 
  mutate(Prop_year = Sum_weight / Total_weight)

```



```{r}
Pl_weight_year_color <-
prop_weight_year %>% 
 ggplot(aes(x = Year, y = Prop_year, fill = Species2)) +
  geom_col()+
  labs(x = "Year", y = "Proportion of the diet by weight, %", fill = "")



```


```{r}
Pl_weight_year <- 
  prop_weight_year %>% 
  ggplot(aes(x = Year, y = Prop_year)) +
  geom_col_pattern(
    aes(
      fill = Species2,
      pattern = Species2,
      pattern_angle = Species2
      ),
    color = "black",
    # key_glyph = "polygon_pattern"
  ) +
  labs(x = "Year", 
       y = "Proportion of the diet by weight") +
  theme_bw() +
  theme(legend.position = "bottom")



# Настройка параметров:
# 1. Тип узора: для первых трёх - "none" (сплошная заливка), для 4 и 5 - "stripe", для 6 - "stipple"

Pl_weight_year <- Pl_weight_year +
  scale_pattern_manual(
  values = c("stripe", "stripe", "none", "none", "none", "none"),
  name = "Species"
) +
  scale_fill_manual(values = c("white", "white", "white", "gray80", "gray40", "black" ), name = "Species")


# 3. Угол наклона: для 4-го - 45° (влево), для 5-го - -45° (вправо), для остальных неважно
Pl_weight_year <- Pl_weight_year +
  scale_pattern_angle_manual(
    values = c(-45, 45, 0, 0, 0, 0),
    name = "Species"
  )

# 4. Объединяем легенды
Pl_weight_year <- Pl_weight_year +
  guides(
    pattern = guide_legend(title = "Species"),
    pattern_fill = guide_legend(title = "Species"),
    pattern_angle = guide_legend(title = "Species")
  )

Pl_weight_year <- 
  Pl_weight_year + 
  scale_y_continuous(labels = scales::percent)

```




```{r}
Pl_weight_doy + Pl_weight_year + 
  plot_annotation(tag_levels = "A") +
  plot_layout(guides = "collect")  &
  theme(legend.position = "bottom")

```



```{r}
Fig_8 <- 
Pl_weight_doy +
  theme(legend.position = "bottom",
        legend.key.size = unit(1, "cm"))  # можно также "mm" или "lines"

Fig_8  
```



```{r}
ggsave(filename = "Figures/Fig_8.tif", plot = Fig_8, dpi = 1200)

```




```{r}
Fig_9 <- 
Pl_weight_year +
  theme(legend.position = "bottom",
        legend.key.size = unit(1, "cm"))  # можно также "mm" или "lines"
Fig_9  
```



```{r}
ggsave(filename = "Figures/Fig_9.tif", plot = Fig_9, dpi = 1200)

```
