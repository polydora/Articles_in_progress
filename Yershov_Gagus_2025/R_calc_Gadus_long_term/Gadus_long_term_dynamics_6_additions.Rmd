...
- Прислать абсолютные значения средних длин трески за разные годы (нужен размах колебаний). Какова среднемноголетняя величина размеров трески, приведенная на рисунке?  
...
- На оси ординат (Length, cm) нанести более дробные значения длины (по 5 см), чтобы было понятно среднемноголетнее значение и размах колебаний по годам. 

**Сделал**

- Разобраться с сезонной динамикой размерного состава выборок

Иного способа, кроме построения модели, где зависимой переменной будет длина, а предикторами год и DOY, не вижу. Проблема будет в том, что модель, конечно покажет значимую связь с годом.

```{r}
Model_L <-  gam(L ~ s(Year) + s(DOY), data = gadus)
```
