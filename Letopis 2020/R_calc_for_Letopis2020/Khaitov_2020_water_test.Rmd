...
В ходе ежегодных учетов гнездящихся птиц на островах вершины Кандалакшского залива проводится определение срока насиживания яиц. Для этих целей используется так называемый "водный тест". В каждом гнезде проводится обследование нескольких яиц (если численность кладки не превышает трех яиц, то тестируются все яйца, если число яиц превышает три, то делается случайная выборка из трех яиц). Процедуре подлежат лишь яйца с цельной скорлупой. Если яйцо надбито, имеет наклевку или проклев (Корякин, Горяшко, 1995), то эта информация фиксируется без дальнейшего использования водного теста, так как по наличию наклевки или проклева уже можно уверенно судить о сроках насиженности.
...
Поскольку данные, приведенные в таблице (`r figRef("wt_table")`) и данные из работы В.В.Бианки и соавторов (1979, были сняты данные с графика, приведенного в работе) несколько разнились, оба источника информации были объединены (Табл. +.1, +.2) и использованы для подбора двух регрессионный моделей. Первая модель описывает связь сроков насиживания с углом наклона яйца. Вторая модель - связь сроков насиживания с диаметром надводной части яйца, опущенного в воду. Модели имеют следующий вид.
...
Сбор материала проводился на островах вершины Кандалакшского залива. Для анализа пространственного распределения величины, характеризующей сроки начала инкубации, были найдены координаты центральной части островов вершины Кандалакшского залива и других географических объектов, к которым происходит привязка учетов гнездящихся птиц (Табл. +.1).
...
В среднем, при обобщении всех данных, откладка яиц происходит на `r Total_median` день от начала календарного года, что приблизительно соответствует `r paste(format(as.Date(156, format = "%j", origin = "1.1.2021"), "%d %b"), "я", sep = "")`. Однако в отдельных географических локациях наблюдается заметное отклонение от этой даты. Средние значения количества дней, прошедших от начала календарного года до откладки яиц, и величины отклонения от среднего по всему архипелагу для каждой точки приведены в таблице +.4. Визуализация приведенных в таблице +.4 данных (`r figRef("Date_begin_map")`) позволяет выявить отчетливый паттерн - на островах, расположенных в открытой части акватории инкубация яиц начинается на несколько дней раньше, чем в среднем по всей акватории. В шхерных же районах начало инкубации задерживается. Вероятной причиной наблюдаемого разброса сроков начала инкубации можно считать сроки освобождения от льда, который в шхерных районах сходит позднее.  
...
kable(wtest_generalized_print, col.names = c("Остров", "Количество проанализированных яиц","Среденее значение даты начала откладки яиц", "Стандартное отклонение", "Отклонение от многолетнего среднего значения"), caption = "Табл. +.4. Средние значения сроков начала откладки яиц на разных островах. Mean values of the beginning of egg laying date on different islands.")
```
...
```{r fig.cap = figRef("Date_begin_map", "Отклонение от многолетней средней даты начала инкубации яиц на островах Вершины Канадалакшского залива. Deviation from the long-term mean start date of egg incubation on the islands of the Kandalaksha Bay") }

load("Data/Kand_upper_map.RData")

ggmap(Kand_map, darken=0, base_layer=ggplot(aes(x=Long, y=Lat), data=wtest_generalized)) + geom_point(aes(color = Anomalia), size = 4)  + scale_color_gradient(low = "white", high = "red", breaks = c(-5, 0, 5, 10)) + labs(color = "Отклонение \nот многолетнего \nсреднего (дни)") + theme(axis.text = element_blank(), axis.title = element_blank(), axis.ticks = element_blank())


```


Обобщение данных за каждый год (`r figRef("DOY_dynam")`) позволяет заметить, что в среднем сроки начала откладки яиц близки к многолетнему среднему значению. Вместе с тем, заметно, что в 2018 году реконструированные по водному тесту сроки начала откладки яиц были смещены на более ранние сроки по сравнению с предыдущими годами, а в 2020 году наблюдалось заметно более позднее начало откладки яиц, чем в предыдущие годы.  


```{r  fig.cap = figRef("DOY_dynam", "Сроки начала откладки яиц на островах Вершины Кандалакшского залива в разные годы. Пунктирная линия отражает многолетнее среднее значение даты откладки яиц. Горизонтальные сплошные линии отражают медианы сроков откладки яиц. Боксы отражают границы 1 и 3 квартилей, размах усов охватывает 95% всех значений в выборке, точки - отскакивающие значения. Dates of the beginning of egg laying on the islands  in the Kandalaksha Bay in different years. The dotted line represents the long-term mean of egg-laying dates.Horizontal solid lines represent medians of egg laying dates in a given year. Boxes represent the boundaries of 1 and 3 quartiles, the whiskers cover 95% of all values variation in the sample, and dots represent outlayers") }


ggplot(wtest_coord, aes(Year, DOY, group = Year)) + 
  geom_boxplot() + 
  geom_hline(yintercept = Total_median, linetype = 2) + 
  scale_x_continuous(breaks = seq(min(wtest_coord$Year), max(wtest_coord$Year)))+
  labs(x = "Годы", y = "Дни от начала календарного года")
...
kable(wtest_print2, col.names = c("Дата учета", "Точка учета","Номер гнезда",  "Количество яиц в гнезде", "Измеренный парамтер"), caption = "Таблица +.5. Измеренные параметры водного теста и расчетные даты откладки яиц. A - Угол; D - диаметр надводной части; В скобках расчетная продолжительность инкубации / Количество дней, прошедших от начала календарного года до откладки данного яйца. Measured water test parameters and estimated egg laying dates.")
